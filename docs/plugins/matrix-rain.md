...
A continuous digital-rain field in the style of the film's code: RedXe's own glyph set of mirrored katakana-like shapes, digits, and a few Latin letters and symbols, falling as bright-headed streams that light the glyphs in place, two streams to a column, with an uneven shimmer and the occasional glyph change. On the Release first page it already fills the canvas, so double-activate does not raise it. On a smaller gallery tile, double-click or double-tap raises it to the full window.
