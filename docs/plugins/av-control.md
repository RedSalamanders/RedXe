...
Smallest useful tile is 160×180 DIPs. Mute, camera and Profile targets are at least 48×48 DIPs. Device icons stay 32 DIP inside the cards. Volume sliders pair a slim rounded track with a clearly visible blue thumb and subtle gray surround. The invisible touch area is 48 DIP tall: grab anywhere near the thumb without jumping the level, or tap farther along the track to seek. Dragging previews the level; releasing commits it. An interrupted touch cancels safely, and changes to the other audio device do not interrupt your drag. Displayed mute and confirmed level state changes only after Windows acknowledges the action. Failure stays visible; it is never drawn as a successful mute or profile switch.
...
At the smallest tile size, the microphone percentage is hidden to keep the thumb and mute button clear. Its level remains available to accessibility tools; enlarge the tile to see both percentages.
