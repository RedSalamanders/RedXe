...
`5h4d3r5.png` (Seascape, the default shader, at the default half render scale) is a `--widget 1` capture from a `--dock bottom@primary --dock-thickness 480` bar on a 4K monitor at 150 %: the scene's page is three columns weighted 1:4:1 with the 5H4D3R5 tile in the middle, so that tile is exactly the XENEON's 2560×720 pixels, then scaled to half size. A bar keeps the window entirely on one monitor, which the capture needs. The fourteen thumbnails under `5h4d3r5/` are the same capture for each `shader` name in `single` mode (`--after 9000`, the fluid solver 24000 and the flames 14000 so their simulations have developed), scaled to 480×135.
