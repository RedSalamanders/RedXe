...
ControlWorkQueue::~ControlWorkQueue()
{
    Stop();
}
...
HRESULT ControlWorkQueue::Start(Notify notify, void* context) noexcept
{
    ...
    try
    {
        _worker = std::thread([this]() noexcept { Worker(); });
    }
        ...
        for (;;)
        {
            ...
            wil::com_ptr_nothrow<IRedXeControlWork> work;
            {
                ...
                for (size_t offset = 1; offset <= _slots.size(); ++offset)
                {
                    ...
                    if (_slots[i].state != State::Queued)
                        continue;
                    selected = i;
                    _lastSelected = i;
            ...
            const HRESULT result = FAILED(apartment) ? apartment
                                   : age >= 3000     ? HRESULT_FROM_WIN32(ERROR_TIMEOUT)
...
{
    if (!_completed.exchange(false, std::memory_order_acq_rel))
        return;
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        if (ready[i].rerun && Running())
            (void)Enqueue(ready[i].work.get());
    }
}
