...
a main stream and, on about half of them, a shorter sparser second stream, each with its own length, speed, and
phase; glyphs stay put in the grid and light as a head passes. Each cell has its own brightness (55–100 %), a slow
±8 % flicker, and a mutation schedule that re-rolls its glyph every `16 / mutationPerSecond` seconds at its own
phase with a brief flash, so the field sparkles unevenly. Stream phase, speed, trail length, column permutation, and
mutation are deterministic functions of the configured seed and frame time. In a tile shorter than the configured
trail (a dock bar, a small gallery tile) the streamed trail length shrinks to two thirds of the visible rows (at
least four glyphs) and the off-screen cycle padding to a third of them (two through eight), so the bright head
crosses the tile for most of each cycle instead of hanging below it; both are grid-cache values, recomputed only with
the grid.
