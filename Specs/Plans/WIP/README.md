...
| [`RFC_UI_ScreenEdgeDock.md`](RFC_UI_ScreenEdgeDock.md) | `DECISION` | `Specs/UI/UI_XeneonDisplayWindowing.md`, `Specs/UI/UI_Dashboard.md`, `Specs/Core/Core_Settings.md`, `Specs/Core/Core_PerformanceAndResources.md`, and `Specs/Plugins/Plugins_Actions.md` | Dock RedXe as a fixed or auto-hiding bar on one edge of any monitor: the `dock` settings object (minor 2), `--dock*` switches, work-area reservation on or off, a peek strip that reveals on mouse dwell, touch, or `redxe.dock.*`. Proposed only; nothing implemented. |
