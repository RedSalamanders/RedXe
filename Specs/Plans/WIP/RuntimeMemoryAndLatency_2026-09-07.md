...
| ID | Item | Pass condition |
| --- | --- | --- |
| P1.1 | **Adapter-of-output device.** `Renderer::CreateDevice` takes the window's `HMONITOR` (`MonitorFromWindow(_window, MONITOR_DEFAULTTONEAREST)`), enumerates `IDXGIFactory1::EnumAdapters1` → `IDXGIAdapter1::EnumOutputs` → `IDXGIOutput::GetDesc().Monitor`, skips `DXGI_ADAPTER_FLAG_SOFTWARE`, and creates with `D3D11CreateDevice(adapter, D3D_DRIVER_TYPE_UNKNOWN, …)`. Store the adapter LUID and monitor. Fall back to today's default-adapter path only when no output matches (hidden test HWND at −32000, headless), then WARP as today. The swap chain factory is the chosen adapter's parent (already so at `Renderer.cpp:387-395`). | `device-created` reports `sameAdapterAsOutput = true`; GPU counters show one LUID; no `nvwgf2umx.dll`/`nvgpucomp64.dll` mapped; PresentMon `Hardware: Independent Flip` on Matrix Focus; UI-thread CPU on Matrix Focus < 1% of a core; threads −37 or better. |
| P1.2 | **Device follows the window.** `Renderer::AdapterOwnsMonitor(HMONITOR)` compares LUIDs. `Application` re-checks on `WM_DISPLAYCHANGE`, `WM_DPICHANGED`, `WM_EXITSIZEMOVE`, and on `WM_MOVE` when `MonitorFromWindow` changed since the last check; a mismatch runs the existing `CreateDeviceResources()` rebuild (`Renderer.cpp:272-325`, the same path `RecoverDevice` uses, which already notifies widgets through `OnDeviceLost`/`OnDeviceCreated`). Release popup, Debug titled window, and XENEON re-placement all use the same rule. | Dragging the Debug window from the 4K display to the XENEON and back rebuilds the device on the target adapter each time (log shows two `device-created` records, LUIDs differ); no rebuild on moves within one adapter. |
| P1.3 | **Waitable swap chain.** `DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT` (also passed to `ResizeBuffers` at `Renderer.cpp:722`), `IDXGISwapChain2::SetMaximumFrameLatency(1)`, drop the device-level call at `:381`. `Application::WaitUntilMessage` (`Application.cpp:3600-3639`) passes the waitable handle as the single object to `MsgWaitForMultipleObjectsEx` when the selected action is `Render`; input is drained before prepare/render; `Present` no longer blocks. Scheduler table unchanged (`FrameScheduler.h`). | ETW shows no blocking inside `Present`; PresentMon input-to-present ≤ 2 frames; `HostPluginTests` scheduler table green. |
| P1.4 | **No redirection surface.** Add `WS_EX_NOREDIRECTIONBITMAP` to the top-level style (`Application.cpp:1043`); the main HWND never paints with GDI (only the edge and overlay child HWNDs do, `:1840`, `:3437`). | Edge bands, raise chrome, settings dialog, and the hidden-window tests behave as before; DWM memory for the window drops by one 7 MiB surface (PresentMon/`DwmGetCompositionTimingInfo`). |
...
| ID | Item | Pass condition |
| --- | --- | --- |
| P5.1 | File DxUi issues (no fork, no pin change): release surface on hide/zero extent; conditional `MarkDirty` in `AdvanceAnimation`; caret tick only on blink flips; cached shadow effects; bounded brush/text-format caches; `cppAllocations` gate in the benchmark; enforce the 128 MiB replacement ceiling. | Issue links recorded here; RedXe keeps the P4.4 workarounds until the pin advances. |
