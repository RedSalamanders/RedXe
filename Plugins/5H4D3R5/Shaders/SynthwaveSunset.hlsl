...
// Pseudo-tessellation noise terrain under a synthwave sky.
...
// HLSL port for RedXe. The optional audio-visualizer path (iChannel0 sound texture) stays disabled as in the source.
...
#define textureMirror(a, b) vec4(0.0, 0.0, 0.0, 0.0)
#else
vec4 textureMirror(Texture2D tex, vec2 c){
    vec2 cf = fract(c);
...
float starnoise(vec3 rd){
    ...
    for (float i=0.;i<4.;i++)
    {
        ...
        c += c2;
        p = p*.6+.5*mul(float3x3(3./5.,0,4./5.,0,1,0,-4./5.,0,3./5.), p);
    }
    c*=c;
