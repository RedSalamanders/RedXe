...
// HLSL port for RedXe. On Shadertoy iChannel0 is a stock photograph behind the wet glass; RedXe does not ship that
// photograph and binds its own procedural night-street stand-in (mipmapped, repeat wrap) instead. The music
// channels have no effect on the picture and are not bound.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    #ifdef CHEAP_NORMALS
    vec2 n = vec2(ddx(c.x), ddy(c.x));// cheap normals (3x cheaper, but 2 times shittier ;))
    #else
    vec2 e = vec2(.001, 0.);
