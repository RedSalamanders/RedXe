// "Base warp fBM" by trinketMage - https://www.shadertoy.com/view/tdG3Rd
// Base for warp fBM (Fractional Brownian Motion) implemented from articles from @iq
// https://iquilezles.org/articles/warp/warp.htm, with the transform_rose colormap from colormap-shaders.
...
// HLSL port for RedXe. The Shadertoy noise texture on iChannel0 is unused by the source (its noise is procedural).
...
float fbm( vec2 p )
{
    ...
    f += 0.500000*noise( p + iTime ); p = mul(p, mtx)*2.02;
