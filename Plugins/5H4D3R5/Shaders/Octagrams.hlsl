// "Octagrams" by whisky_shusuky - https://www.shadertoy.com/view/tlVGDt
...
// HLSL port for RedXe. The unused `iTime` parameters of box_set/map are renamed so they do not shadow the uniform.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    ray.xy = mul(rot(sin(iTime * .03) * 5.), ray.xy);
    ray.yz = mul(rot(sin(iTime * .05) * .2), ray.yz);
    float t = 0.1;
    vec3 col = vec3(0.,0.,0.);
    float ac = 0.0;
    ...
    col = vec3(ac * 0.02, ac * 0.02, ac * 0.02);
