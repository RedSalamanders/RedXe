...
// HLSL port for RedXe. The ray globals are gRo/gRd so the helpers' (ro, rd) parameters keep their names, the three
// light loops use distinct counters (fxc rejects a reused loop variable), the music channel has no effect on the
// picture and is not bound, and iMouse stays zero.
...
vec2 GetDrops(vec2 uv, float seed, float m) {
    ...
    #ifdef DROP_DEBUG
    if(uv2.x<.02 || uv2.y<.01) o = vec2(1.,1.);
    #endif
...
vec3 HeadLights(float i, float t) {
    ...
    float d = length(p-gRo);

    float size = mix(.03, .05, S(.02, .07, z))*d;
    float m = 0.;
    float blur = .1;
    m += BokehMask(gRo, gRd, p-vec3(.08, 0., 0.), size, blur);
    m += BokehMask(gRo, gRd, p+vec3(.08, 0., 0.), size, blur);

    #ifdef HIGH_QUALITY
    m += BokehMask(gRo, gRd, p+vec3(.1, 0., 0.), size, blur);
    m += BokehMask(gRo, gRd, p-vec3(.1, 0., 0.), size, blur);
    #endif
    ...
    size *= 2.5;
    float r = 0.;
    r += BokehMask(gRo, gRd, p+vec3(-.09, -.2, 0.), size, blur);
    r += BokehMask(gRo, gRd, p+vec3(.09, -.2, 0.), size, blur);
    r *= distFade*distFade;
...
vec3 TailLights(float i, float t) {
    ...
    float size = .05*d;
    float blur = .1;
    float m = BokehMask(gRo, gRd, p-vec3(.08, 0., 0.), size, blur) +
                BokehMask(gRo, gRd, p+vec3(.08, 0., 0.), size, blur);
    ...
    m += (BokehMask(gRo, gRd, p+vec3(.1, 0., 0.), size, blur) +
        BokehMask(gRo, gRd, p-vec3(.1, 0., 0.), size, blur))*brake;
    #endif

    float refSize = size*2.5;
    m += BokehMask(gRo, gRd, p+vec3(-.09, -.2, 0.), refSize, .8);
    m += BokehMask(gRo, gRd, p+vec3(.09, -.2, 0.), refSize, .8);
    vec3 col = tailLightCol*m*ft;

    float b = BokehMask(gRo, gRd, p+vec3(.12, 0., 0.), size, blur);
    b += BokehMask(gRo, gRd, p+vec3(.12, -.2, 0.), refSize, .8)*.2;
...
vec3 EnvironmentLights(float i, float t) {
    ...
    vec3 rp = ClosestPoint(gRo, gRd, p);
    float distFade = Remap(1., .7, .1, 1.5, 1.-pow(1.-z,6.));
    float m = BokehMask(gRo, gRd, p, .05*d, blur);
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    [loop]
    for(float j=0.; j<1.; j+=stp) {
        float n = N(j+floor(t));
        col += HeadLights(j+n*stp*.7, t);
    }
    ...
    [loop]
    for(float k=0.; k<1.; k+=stp) {
       col += EnvironmentLights(k, t);
    }

    col += TailLights(0., t);
    col += TailLights(.5, t);

    col += sat(gRd.y)*vec3(.6, .5, .9);
