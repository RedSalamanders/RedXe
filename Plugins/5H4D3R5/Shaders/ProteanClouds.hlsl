...
// HLSL port for RedXe. iMouse stays zero, which is the view Shadertoy shows before any click; that framing is
// anchored to a 16:9 canvas (see mainImage) so a wide tile keeps it.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // RedXe: with no mouse the source frames its camera from the canvas half-size, so on a 32:9 tile the offsets
    // double and the view leaves the clouds; anchor them to the 16:9 canvas Shadertoy shows this shader on.
    bsMo = (iMouse.xy - 0.5*vec2(iResolution.y*16.0/9.0, iResolution.y))/iResolution.y;
