 ...
 */
// https://www.shadertoy.com/view/Ms2SD1 - fully-procedural sea surface computing, without textures.
// HLSL port for RedXe. GLSL row-vector matrix products became mul(matrix, vector) with the same coefficients.
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // post (RedXe: a trough can push a channel slightly below zero; pow() of a negative base is undefined in GLSL
    // and NaN in HLSL, and on hardware Direct3D that NaN surfaced as saturated pink patches, so clamp first)
    fragColor = vec4(pow(max(color, 0.0),vec3(0.65, 0.65, 0.65)), 1.0);
}
