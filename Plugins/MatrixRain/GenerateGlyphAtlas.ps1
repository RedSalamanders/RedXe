...
an exact signed distance field into a 24 x 48 texel cell, 8 x 8 cells in a 192 x 384 R8_UNORM atlas. The set is the
...
$strokeRadius = 0.095                     # units; about 2.3 px in the atlas, 2.7 px at a 27 px glyph
...
for ($glyph = 0; $glyph -lt $glyphs.Count; ++$glyph) {
    ...
    foreach ($polyline in $polylines) {
        ...
        for ($index = 0; $index -lt [Math]::Max(1, $pointCount - 1); ++$index) {
            $ax = $points[2 * $index]; $ay = $points[2 * $index + 1]
            $bx = if ($pointCount -gt 1) { $points[2 * $index + 2] } else { $ax }
            $by = if ($pointCount -gt 1) { $points[2 * $index + 3] } else { $ay }
...
[void]$builder.AppendLine('// eight Latin letters and symbols) as an exact signed distance field, 24 x 48 texels per glyph in an 8 x 8 grid,')
