...
void BridgeServer::Client::Close() noexcept
{
    if (pending && pipe)
    {
        ...
        DWORD ignored = 0;
        (void)GetOverlappedResult(pipe.get(), &read, &ignored, TRUE);
    }
    pending = false;
...
uint32_t BridgeServer::DemandingConsumers() noexcept
{
    ...
    for (const auto& client : _clients)
        if (client.pipe && client.demandedAt && now - client.demandedAt < 250)
            ++result;
    return result;
}
...
void BridgeServer::Stop() noexcept
{
    ...
    if (_connecting && _listener)
    {
        ...
        DWORD ignored = 0;
        (void)GetOverlappedResult(_listener.get(), &_connect, &ignored, TRUE);
    }
    _connecting = false;
    _listener.reset();
...
HRESULT BridgeServer::Listen(bool first) noexcept
{
    ...
    _listener.reset(CreateNamedPipeW(
        _identity.pipeName.c_str(),
...
DWORD WINAPI BridgeServer::Entry(void* value) noexcept
{
    ...
    if (FAILED(initialized))
        return 1;
    static_cast<BridgeServer*>(value)->Run();
    CoUninitialize();
    return 0;
}
    ...
    for (;;)
    {
        ...
        DWORD timeout = INFINITE;
        {
            ...
            for (auto& client : _clients)
                if (client.pipe)
                {
                    clients[count - 3] = &client;
                    handles[count++] = client.event.get();
        ...
        if (wait == WAIT_TIMEOUT)
        {
            ...
            const auto now = GetTickCount64();
            for (auto& client : _clients)
                if (client.demandedAt && now - client.demandedAt >= 250)
                    client.demandedAt = 0;
        ...
        else if (wait >= WAIT_OBJECT_0 + 3 && wait < WAIT_OBJECT_0 + count)
        {
            ...
            DWORD received = 0;
            if (!client.closeRequested && GetOverlappedResult(client.pipe.get(), &client.read, &received, FALSE) &&
                received == 1 && client.unexpected == 1)
            {
                client.pending = false;
                client.demandedAt = GetTickCount64();
                client.ReadDemand();
            }
            else
                Drop(client); // End-of-file or an unknown control byte terminates this lease.
        }
        if (_observer)
            _observer->OnBridgeWork();
    }
    if (_observer)
        _observer->OnBridgeStopping();
}
...
void BridgeServer::Accept() noexcept
{
    ...
    if (_consumers >= MaximumConsumers)
        pipe.reset();
    if (FAILED(Listen(false)))
    {
        SetEvent(_stop.get());
        return;
    }
    ...
    if (SUCCEEDED(reply.status) &&
        (hello.magic != BridgeMagic || hello.version != BridgeVersion || hello.size != sizeof(hello) || hello.reserved))
        reply.status = HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    if (SUCCEEDED(reply.status))
        reply.status = AuthenticateBridgeConsumer(pipe.get(), _identity);
    const auto lock = wil::AcquireSRWLockExclusive(&_lock);
    ...
    if (!candidate)
        reply.status = HRESULT_FROM_WIN32(ERROR_TOO_MANY_SESS);
    if (candidate && SUCCEEDED(reply.status))
    {
        reply.status = candidate->channel.Open(hello.channel, _identity.synthetic);
        ChannelState state;
        if (SUCCEEDED(reply.status))
            reply.status = candidate->channel.State(state);
        if (SUCCEEDED(reply.status))
        {
            if (state.revision == UINT64_MAX)
                reply.status = HRESULT_FROM_WIN32(ERROR_ARITHMETIC_OVERFLOW);
            else
            {
                candidate->channelRevision = state.revision + 1;
                reply.status = candidate->channel.SetGate({candidate->channelRevision, _enabled});
            }
        }
    }
    ...
    if (!candidate)
        return;
    if (FAILED(sent) || FAILED(reply.status))
    {
        candidate->Close();
        return;
    }
    ...
    candidate->pipe = std::move(pipe);
    candidate->ReadDemand();
    ++_consumers;
}
HRESULT BridgeServer::SetGate(uint64_t generation, bool enabled) noexcept
{
    ...
    for (auto& client : _clients)
        if (client.pipe)
        {
            const HRESULT changed = client.channelRevision == UINT64_MAX
            ...
            if (FAILED(changed))
            {
                ...
                (void)CancelIoEx(client.pipe.get(), &client.read);
                SetEvent(client.event.get());
            }
        }
    Wake();
    return result;
