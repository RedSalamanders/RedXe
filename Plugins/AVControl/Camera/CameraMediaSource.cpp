...
bool IsSupportedType(IMFMediaType* type) noexcept
{
    ...
    return SUCCEEDED(type->GetGUID(MF_MT_MAJOR_TYPE, &major)) && major == MFMediaType_Video &&
        ...
        SUCCEEDED(MFGetAttributeSize(type, MF_MT_FRAME_SIZE, &width, &height)) && width == FrameWidth && height == FrameHeight &&
        SUCCEEDED(MFGetAttributeRatio(type, MF_MT_FRAME_RATE, &numerator, &denominator)) && numerator == FrameRate && denominator == 1 &&
        SUCCEEDED(type->GetUINT32(MF_MT_INTERLACE_MODE, &interlace)) && interlace == MFVideoInterlace_Progressive;
}
HRESULT Neutral(BYTE* bytes, DWORD capacity, LONG pitch) noexcept
{
    if (!bytes || pitch < static_cast<LONG>(FrameWidth) ||
        static_cast<uint64_t>(pitch) * FrameHeight * 3 / 2 > capacity) return E_INVALIDARG;
...
class Stream final : public RedXeComObject<Stream, IMFMediaStream2, IMFAsyncCallback>, private Counted
{
  public:
    ...
    {
        _parent = parent; _provider = std::move(provider);
    ...
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID iid, void** output) noexcept override
    {
        return RedXeComObject::QueryInterface(iid == __uuidof(IMFMediaStream) || iid == __uuidof(IMFMediaEventGenerator) ?
            __uuidof(IMFMediaStream2) : iid, output);
    }
    HRESULT STDMETHODCALLTYPE GetEvent(DWORD flags, IMFMediaEvent** result) noexcept override
    {
        ...
        { const auto lock = wil::AcquireSRWLockShared(&_lock); if (_shutdown) return MF_E_SHUTDOWN; queue = _events; }
    ...
    HRESULT STDMETHODCALLTYPE QueueEvent(MediaEventType type, REFGUID extended, HRESULT status, const PROPVARIANT* value) noexcept override
    {
        const auto lock = wil::AcquireSRWLockShared(&_lock);
    ...
    HRESULT STDMETHODCALLTYPE RequestSample(IUnknown* token) noexcept override
    {
        ...
        if (_count == _tokens.size()) return MF_E_NOTACCEPTING;
        _tokens[(_head + _count) % _tokens.size()] = token; ++_count;
        const HRESULT result = Schedule();
        if (FAILED(result)) { --_count; _tokens[(_head + _count) % _tokens.size()].reset(); }
        return result;
    }
    HRESULT STDMETHODCALLTYPE SetStreamState(MF_STREAM_STATE state) noexcept override
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        if (state == _state) return S_OK;
        if (state == MF_STREAM_STATE_STOPPED) return StopLocked(false);
    ...
    HRESULT STDMETHODCALLTYPE GetStreamState(MF_STREAM_STATE* state) noexcept override
    {
        ...
        const auto lock = wil::AcquireSRWLockShared(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        *state = _state; return S_OK;
    }
    HRESULT SetAllocator(IUnknown* allocator) noexcept
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        if (_state != MF_STREAM_STATE_STOPPED) return MF_E_INVALID_STATE_TRANSITION;
    ...
    HRESULT Start(IMFMediaType* type) noexcept
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        if (!IsSupportedType(type)) return MF_E_INVALIDMEDIATYPE;
        if (_state == MF_STREAM_STATE_RUNNING) return S_OK;
        if (!_allocator) RETURN_IF_FAILED(MFCreateVideoSampleAllocatorEx(IID_PPV_ARGS(_allocator.put())));
        RETURN_IF_FAILED(_allocator->InitializeSampleAllocator(MaximumSamplePool, type));
        _session.attach(new (std::nothrow) Session());
        if (!_session) { (void)_allocator->UninitializeSampleAllocator(); return E_OUTOFMEMORY; }
        const HRESULT started = _provider ? _provider->Start() : S_OK;
        if (FAILED(started)) { _session.reset(); (void)_allocator->UninitializeSampleAllocator(); return started; }
        _state = MF_STREAM_STATE_RUNNING; _lastTimestamp = 0;
        PROPVARIANT time{}; time.vt = VT_I8; time.hVal.QuadPart = MFGetSystemTime();
    ...
    void Shutdown() noexcept
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return;
        (void)StopLocked(false); _shutdown = true;
        if (_events) (void)_events->Shutdown();
        _events.reset(); _parent.reset(); _descriptor.reset(); _allocator.reset(); _provider.reset();
    }
    HRESULT STDMETHODCALLTYPE GetParameters(DWORD* flags, DWORD* queue) noexcept override
    {
        if (!flags || !queue) return E_POINTER;
        *flags = 0; *queue = _workQueue.get(); return S_OK;
    }
    HRESULT STDMETHODCALLTYPE Invoke(IMFAsyncResult* result) noexcept override
    {
        wil::com_ptr_nothrow<IUnknown> session;
        if (!result || FAILED(result->GetState(session.put()))) return E_INVALIDARG;
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        ...
        if (_shutdown || !_session || session.get() != static_cast<IUnknown*>(_session.get())) return S_OK;
        _scheduled = false; _workKey = 0;
        if (_state != MF_STREAM_STATE_RUNNING || _count == 0) return S_OK;
        auto token = std::move(_tokens[_head]); _head = (_head + 1) % _tokens.size(); --_count;
        wil::com_ptr_nothrow<IMFSample> sample;
        HRESULT status = S_OK;
        {
            const auto releaseGate = wil::scope_exit([&] { if (_provider) _provider->EndFrame(); });
            status = FillSample(token.get(), sample.put());
            if (SUCCEEDED(status)) status = _events->QueueEventParamUnk(MEMediaSample, GUID_NULL, S_OK, sample.get());
        }
        if (FAILED(status)) (void)_events->QueueEventParamVar(MEError, GUID_NULL, status, nullptr);
        if (_count) (void)Schedule();
        return S_OK;
    }
  private:
    ...
    {
        if (_scheduled || !_count) return S_OK;
        ...
        _scheduled = true; return S_OK;
    }
    HRESULT StopLocked(bool notify) noexcept
    {
        if (_scheduled) (void)MFCancelWorkItem(_workKey);
        _scheduled = false; _workKey = 0; _session.reset();
        for (auto& token : _tokens) token.reset();
        _head = _count = 0;
        if (_state == MF_STREAM_STATE_RUNNING && _provider) _provider->Stop();
        _state = MF_STREAM_STATE_STOPPED;
        if (_allocator) (void)_allocator->UninitializeSampleAllocator();
    ...
    HRESULT FillSample(IUnknown* token, IMFSample** result) noexcept
    {
        ...
        BYTE* scan = nullptr; BYTE* start = nullptr; DWORD bytes = 0; LONG pitch = 0;
        ...
        if (!scan || !start || scan < start || static_cast<uint64_t>(scan - start) > bytes) return E_UNEXPECTED;
        ...
        if (!_provider || FAILED(_provider->Fill(scan, capacity, pitch, timestamp))) RETURN_IF_FAILED(Neutral(scan, capacity, pitch));
        ...
        if (token) RETURN_IF_FAILED(sample->SetUnknown(MFSampleExtension_Token, token));
        else (void)sample->DeleteItem(MFSampleExtension_Token);
        _lastTimestamp = timestamp;
        *result = sample.detach(); return S_OK;
    }
};

class Source final : public RedXeComObject<Source, IMFMediaSourceEx, IMFGetService, IKsControl, IMFSampleAllocatorControl>, private Counted
{
  public:
    HRESULT Initialize(std::shared_ptr<FrameProvider> provider) noexcept
    {
        ...
        RETURN_IF_FAILED(_attributes->SetGUID(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE, MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_GUID));
    ...
    HRESULT STDMETHODCALLTYPE QueryInterface(REFIID iid, void** result) noexcept override
    {
        return RedXeComObject::QueryInterface(iid == __uuidof(IMFMediaSource) || iid == __uuidof(IMFMediaEventGenerator) ?
            __uuidof(IMFMediaSourceEx) : iid, result);
    }
    HRESULT STDMETHODCALLTYPE GetEvent(DWORD flags, IMFMediaEvent** result) noexcept override
    {
        wil::com_ptr_nothrow<IMFMediaEventQueue> queue;
    ...
    HRESULT STDMETHODCALLTYPE BeginGetEvent(IMFAsyncCallback* callback, IUnknown* state) noexcept override
    { const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : _events->BeginGetEvent(callback, state); }
    HRESULT STDMETHODCALLTYPE EndGetEvent(IMFAsyncResult* result, IMFMediaEvent** event) noexcept override
    { const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : _events->EndGetEvent(result, event); }
    HRESULT STDMETHODCALLTYPE QueueEvent(MediaEventType type, REFGUID extended, HRESULT status, const PROPVARIANT* value) noexcept override
    { const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : _events->QueueEventParamVar(type, extended, status, value); }
    HRESULT STDMETHODCALLTYPE GetCharacteristics(DWORD* result) noexcept override
    {
        if (!result) return E_POINTER;
        const auto lock = wil::AcquireSRWLockShared(&_lock); if (_shutdown) return MF_E_SHUTDOWN;
        *result = MFMEDIASOURCE_IS_LIVE; return S_OK;
    }
    HRESULT STDMETHODCALLTYPE CreatePresentationDescriptor(IMFPresentationDescriptor** result) noexcept override
    {
        if (!result) return E_POINTER; *result = nullptr;
        const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : _presentation->Clone(result);
    }
    HRESULT STDMETHODCALLTYPE Start(IMFPresentationDescriptor* presentation, const GUID* timeFormat, const PROPVARIANT* position) noexcept override
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        if (!presentation || !position) return E_INVALIDARG;
        if (timeFormat && *timeFormat != GUID_NULL) return MF_E_UNSUPPORTED_TIME_FORMAT;
        if (position->vt != VT_EMPTY && (position->vt != VT_I8 || position->hVal.QuadPart != 0)) return MF_E_INVALIDREQUEST;
        DWORD count = 0, id = UINT_MAX; BOOL selected = FALSE;
        wil::com_ptr_nothrow<IMFStreamDescriptor> descriptor;
        wil::com_ptr_nothrow<IMFMediaTypeHandler> handler;
        wil::com_ptr_nothrow<IMFMediaType> type;
        RETURN_IF_FAILED(presentation->GetStreamDescriptorCount(&count));
        if (count != 1) return MF_E_INVALIDSTREAMNUMBER;
        ...
        if (!selected || id != 0) return MF_E_INVALIDSTREAMNUMBER;
        ...
        _everStarted = true; _started = true;
        PROPVARIANT time{}; time.vt = VT_I8; time.hVal.QuadPart = MFGetSystemTime();
    ...
    HRESULT STDMETHODCALLTYPE Stop() noexcept override
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return MF_E_SHUTDOWN;
        if (!_started) return S_OK;
        RETURN_IF_FAILED(_stream->Stop()); _started = false;
    ...
    { const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : MF_E_INVALID_STATE_TRANSITION; }
    HRESULT STDMETHODCALLTYPE Shutdown() noexcept override
    {
        const auto lock = wil::AcquireSRWLockExclusive(&_lock);
        if (_shutdown) return S_OK;
        _shutdown = true; _started = false;
        if (_stream) _stream->Shutdown();
        if (_events) (void)_events->Shutdown();
        _stream.reset(); _events.reset(); _presentation.reset(); _attributes.reset(); return S_OK;
    }
    HRESULT STDMETHODCALLTYPE GetSourceAttributes(IMFAttributes** result) noexcept override
    {
        if (!result) return E_POINTER; *result = nullptr;
        const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : _attributes.copy_to(result);
    }
    ...
    { const auto lock = wil::AcquireSRWLockShared(&_lock); return _shutdown ? MF_E_SHUTDOWN : S_OK; }
    HRESULT STDMETHODCALLTYPE GetService(REFGUID, REFIID, void** result) noexcept override
    { if (!result) return E_POINTER; *result = nullptr; return MF_E_UNSUPPORTED_SERVICE; }
    ...
    HRESULT STDMETHODCALLTYPE GetAllocatorUsage(DWORD id, DWORD* input, MFSampleAllocatorUsage* usage) noexcept override
    {
        ...
        const auto lock = wil::AcquireSRWLockShared(&_lock); if (_shutdown) return MF_E_SHUTDOWN;
        if (id != 0) return MF_E_INVALIDSTREAMNUMBER;
        *input = 0; *usage = MFSampleAllocatorUsage_UsesProvidedAllocator; return S_OK;
    }
  private:
    SRWLOCK _lock = SRWLOCK_INIT;
    wil::com_ptr_nothrow<IMFMediaEventQueue> _events;
...
HRESULT CreateMediaSource(std::shared_ptr<FrameProvider> provider, IMFMediaSource** result) noexcept
{
    ...
    if (FAILED(initialized)) { (void)source->Shutdown(); return initialized; }
    return source.query_to(result);
}
uint32_t ActiveMediaObjects() noexcept { return objectCount.load(); }
} // namespace AVControl::Camera
