...
constexpr LONGLONG MaximumFrameAge =
    2'500'000; // 250 ms; a dead producer can never repeat a stale physical frame forever.
...
bool ValidName(std::wstring_view value, bool mutex, bool allowLocal) noexcept
{
    ...
    if (value.starts_with(globalPrefix))
        value.remove_prefix(globalPrefix.size());
    else if (allowLocal && value.starts_with(localPrefix))
        value.remove_prefix(localPrefix.size());
    else
        return false;
...
HRESULT FrameChannel::Create(PCWSTR ownerSid, bool localTestNamespace) noexcept
{
    ...
    try
    {
        ...
        wchar_t suffix[40]{};
        if (!StringFromGUID2(guid, suffix, static_cast<int>(std::size(suffix))))
            return E_UNEXPECTED;
        ...
        _mapping.reset(CreateFileMappingW(INVALID_HANDLE_VALUE, &attributes, PAGE_READWRITE, 0, sizeof(Shared),
                                          _names.mapping.data()));
        if (!_mapping)
            return HRESULT_FROM_WIN32(GetLastError());
...
HRESULT FrameChannel::Open(const FrameChannelNames& names, bool allowLocalTestNamespace) noexcept
{
    ...
    if (mappingLength == names.mapping.size() || mutexLength == names.mutex.size() ||
        ...
        !ValidName({names.mutex.data(), mutexLength}, true, allowLocalTestNamespace))
        return E_INVALIDARG;
    // Both names must use the same random suffix and namespace, never unrelated locks around shared bytes.
    if (std::wstring_view(names.mapping.data()).substr(mappingLength - 38) !=
            std::wstring_view(names.mutex.data()).substr(mutexLength - 38) ||
        names.mapping[0] != names.mutex[0])
        return E_INVALIDARG;
...
HRESULT FrameChannel::State(ChannelState& result) noexcept
{
    ...
    result = {_view->revision, _view->enabled != 0};
    return S_OK;
}
HRESULT FrameChannel::SetGate(ChannelState state) noexcept
{
    ...
    wil::mutex_release_scope_exit lease;
    RETURN_IF_FAILED(Lock(lease));
    if (state.revision < _view->revision ||
        (state.revision == _view->revision && state.enabled != (_view->enabled != 0)))
    ...
    _view->revision = state.revision;
    _view->enabled = state.enabled ? 1U : 0U;
    return S_OK;
}
HRESULT FrameChannel::Write(uint64_t revision, std::span<const BYTE> nv12, LONGLONG timestamp) noexcept
{
    if (nv12.size() != ImageBytes || timestamp <= 0)
        return E_INVALIDARG;
    wil::mutex_release_scope_exit lease;
    RETURN_IF_FAILED(Lock(lease));
    if (!_view->enabled || revision != _view->revision)
        return HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH);
    if (timestamp <= _view->timestamp)
        return S_FALSE;
...
HRESULT FrameChannel::Read(BYTE* bytes, DWORD capacity, LONG pitch, LONGLONG now) noexcept
{
    if (!bytes || pitch < static_cast<LONG>(FrameWidth) ||
        static_cast<uint64_t>(pitch) * FrameHeight * 3 / 2 > capacity || now <= 0)
        return E_INVALIDARG;
    wil::mutex_release_scope_exit lease;
    RETURN_IF_FAILED(Lock(lease));
    _readLease = std::move(lease);
    const bool live =
        _view->enabled && _view->ready && _view->timestamp <= now && now - _view->timestamp <= MaximumFrameAge;
    for (UINT row = 0; row < FrameHeight * 3 / 2; ++row)
    {
        ...
        if (live)
            std::memcpy(target, _view->image.data() + static_cast<size_t>(row) * FrameWidth, FrameWidth);
    }
    return S_OK;
}
void FrameChannel::EndRead() noexcept
{
    _readLease.reset();
}
} // namespace AVControl::Camera
