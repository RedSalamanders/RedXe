...
HRESULT CurrentOwner(std::wstring& result) noexcept
{
    ...
    RETURN_IF_WIN32_BOOL_FALSE(
        GetTokenInformation(token.get(), TokenUser, bytes.data(), static_cast<DWORD>(bytes.size()), &needed));
    wil::unique_hlocal_string sid;
    RETURN_IF_WIN32_BOOL_FALSE(
        ConvertSidToStringSidW(reinterpret_cast<TOKEN_USER*>(bytes.data())->User.Sid, sid.put()));
    try
    {
        result = sid.get();
        return S_OK;
...
struct VirtualCameraRoute::State final
{
    ...
    HRESULT Create() noexcept
    {
        ...
        if (!module)
            return E_NOTIMPL; // Windows 10 remains loadable; the Windows 11 export is optional.
        ...
        RETURN_IF_FAILED(create(MFVirtualCameraType_SoftwareCameraSource, MFVirtualCameraLifetime_System,
                                MFVirtualCameraAccess_CurrentUser, FriendlyName, CameraSourceClsidString, nullptr, 0,
                                camera.put()));
        return S_OK;
    }
};
VirtualCameraRoute::VirtualCameraRoute() : _state(std::make_unique<State>()) {}
VirtualCameraRoute::~VirtualCameraRoute()
{
    Close();
}
HRESULT VirtualCameraRoute::ProbeInstalledSource() noexcept
{
    ...
    try
    {
        const std::wstring expected =
            std::wstring(programFiles.get()) + L"\\RedSalamanders\\RedXe Camera\\AVControlCamera.dll";
        if (_wcsicmp(expected.c_str(), registered.data()) != 0)
...
{
    if (_state->camera && !_state->symbolicLink.empty())
        return S_OK;
    Close();
    ...
    UINT32 length = 0;
    RETURN_IF_FAILED(_state->camera->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK,
                                                        symbolic.put(), &length));
    if (!length || length > 1024)
        return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    try
    {
        _state->symbolicLink.assign(symbolic.get(), length);
    }
    catch (const std::bad_alloc&)
    {
        return E_OUTOFMEMORY;
...
{
    if (_state->camera)
        (void)_state->camera->Shutdown();
    _state->camera.reset();
    _state->module.reset();
    _state->owner.clear();
    _state->symbolicLink.clear();
...
std::wstring_view VirtualCameraRoute::SymbolicLink() const noexcept
{
    return _state->symbolicLink;
}
std::wstring_view VirtualCameraRoute::OwnerSid() const noexcept
{
    return _state->owner;
}
} // namespace AVControl::Camera
