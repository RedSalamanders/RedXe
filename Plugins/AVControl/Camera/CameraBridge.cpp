...
HRESULT TokenMatches(HANDLE token, PSID owner, bool serviceOnly) noexcept
{
    ...
    DWORD needed = 0;
    RETURN_IF_WIN32_BOOL_FALSE(GetTokenInformation(token, TokenUser, data.data(), static_cast<DWORD>(data.size()), &needed));
    const auto user = reinterpret_cast<TOKEN_USER*>(data.data());
    if (!serviceOnly && EqualSid(user->User.Sid, owner)) return S_OK;
    if (!IsWellKnownSid(user->User.Sid, WinLocalServiceSid)) return E_ACCESSDENIED;
...
HRESULT MakeBridgeIdentity(PCWSTR ownerSid, BridgeIdentity& result, const GUID* isolatedTest) noexcept
{
    ...
    try
    {
        ...
        {
            wchar_t guid[40]{};
            if (!StringFromGUID2(*isolatedTest, guid, static_cast<int>(std::size(guid)))) return E_INVALIDARG;
            next.pipeName += L".Test."; next.pipeName += guid; next.synthetic = true;
        }
        if (next.pipeName.size() >= 256) return E_INVALIDARG;
...
HRESULT CreateBridgeSecurity(const BridgeIdentity& identity, wil::unique_hlocal& security) noexcept
{
    try
    {
        ...
        const std::wstring sddl = L"O:" + identity.ownerSid + L"D:P(A;;GA;;;SY)(A;;GA;;;LS)(A;;GA;;;" + identity.ownerSid + L")";
        RETURN_IF_WIN32_BOOL_FALSE(ConvertStringSecurityDescriptorToSecurityDescriptorW(sddl.c_str(), SDDL_REVISION_1, security.put(), nullptr));
        return S_OK;
    }
    catch (const std::bad_alloc&) { return E_OUTOFMEMORY; }
}
HRESULT AuthenticateBridgeServer(HANDLE pipe, const BridgeIdentity& identity) noexcept
{
    ...
    const DWORD error = GetSecurityInfo(pipe, SE_KERNEL_OBJECT, OWNER_SECURITY_INFORMATION, &owner, nullptr, nullptr, nullptr,
        reinterpret_cast<PSECURITY_DESCRIPTOR*>(security.put()));
...
HRESULT AuthenticateBridgeConsumer(HANDLE pipe, const BridgeIdentity& identity) noexcept
{
    ...
    const auto revert = wil::scope_exit([] { if (!RevertToSelf()) std::terminate(); });
...
{
    if (!pipe || pipe == INVALID_HANDLE_VALUE || !bytes || !size) return E_INVALIDARG;
    ...
    while (total < size)
    {
        ...
        const BOOL immediate = write ? WriteFile(pipe, static_cast<BYTE*>(bytes) + total, size - total, nullptr, &operation) :
            ReadFile(pipe, static_cast<BYTE*>(bytes) + total, size - total, nullptr, &operation);
        if (!immediate && GetLastError() != ERROR_IO_PENDING) return HRESULT_FROM_WIN32(GetLastError());
        if (!immediate)
        {
            const ULONGLONG now = GetTickCount64();
            const DWORD remaining = static_cast<DWORD>(std::min<ULONGLONG>(deadline > now ? deadline - now : 0, BridgeDeadlineMs));
            if (WaitForSingleObject(event.get(), remaining) != WAIT_OBJECT_0)
            {
                (void)CancelIoEx(pipe, &operation);
                DWORD ignored = 0; (void)GetOverlappedResult(pipe, &operation, &ignored, TRUE);
                return HRESULT_FROM_WIN32(ERROR_TIMEOUT);
            }
        }
        DWORD transferred = 0;
        RETURN_IF_WIN32_BOOL_FALSE(GetOverlappedResult(pipe, &operation, &transferred, FALSE));
        if (!transferred || transferred > size - total) return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
        total += transferred;
    }
    return S_OK;
}
...
void BridgeFrameProvider::Disconnect() noexcept
{
    if (_demandPending && _pipe)
    {
        ...
        DWORD ignored = 0; (void)GetOverlappedResult(_pipe.get(), &_demand, &ignored, TRUE);
    }
    _demandPending = false; _demandEvent.reset(); _demand = {};
    _pipe.reset();
    ChannelState state;
    if (SUCCEEDED(_channel.State(state)) && state.revision != UINT64_MAX) (void)_channel.SetGate({state.revision + 1, false});
    _retryAt = GetTickCount64() + 1000;
}
void BridgeFrameProvider::Stop() noexcept
{
    _channel.EndRead(); Disconnect(); _channel.Close(); _started = false; _retryAt = 0;
}
HRESULT BridgeFrameProvider::Connect() noexcept
{
    _retryAt = GetTickCount64() + 1000;
    _pipe.reset(CreateFileW(_identity.pipeName.c_str(), GENERIC_READ | GENERIC_WRITE | READ_CONTROL, 0, nullptr, OPEN_EXISTING,
        FILE_FLAG_OVERLAPPED | SECURITY_SQOS_PRESENT | SECURITY_IDENTIFICATION, nullptr));
    if (!_pipe) return HRESULT_FROM_WIN32(GetLastError());
    auto fail = wil::scope_exit([&] { Disconnect(); });
    RETURN_IF_FAILED(AuthenticateBridgeServer(_pipe.get(), _identity));
    BridgeHello hello; hello.channel = _channel.Names(); BridgeReply reply;
    ...
    if (reply.magic != BridgeMagic || reply.version != BridgeVersion || reply.size != sizeof(reply)) return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    RETURN_IF_FAILED(reply.status);
    _demandEvent.reset(CreateEventW(nullptr, TRUE, FALSE, nullptr));
    if (!_demandEvent) return HRESULT_FROM_WIN32(GetLastError());
    _demand = {}; _demand.hEvent = _demandEvent.get();
    fail.release(); return S_OK;
}
HRESULT BridgeFrameProvider::Demand() noexcept
{
    ...
    if (_demandPending)
    {
        ...
        if (written != 1) return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    }
    ResetEvent(_demandEvent.get()); _demand = {}; _demand.hEvent = _demandEvent.get();
    if (WriteFile(_pipe.get(), &_demandByte, 1, nullptr, &_demand)) return S_OK;
    if (GetLastError() != ERROR_IO_PENDING) return HRESULT_FROM_WIN32(GetLastError());
    _demandPending = true; return S_OK;
}
HRESULT BridgeFrameProvider::Fill(BYTE* bytes, DWORD capacity, LONG pitch, LONGLONG timestamp) noexcept
{
    if (!_started) return MF_E_SHUTDOWN;
    if (_pipe)
    {
        DWORD available = 0;
        if (!PeekNamedPipe(_pipe.get(), nullptr, 0, nullptr, &available, nullptr) || available) Disconnect();
    }
    if (!_pipe && GetTickCount64() >= _retryAt) (void)Connect();
    if (_pipe && FAILED(Demand())) Disconnect();
