...
Availability FailureState(HRESULT result) noexcept
{
    ...
    if (result == E_ACCESSDENIED || result == HRESULT_FROM_WIN32(ERROR_ACCESS_DISABLED_BY_POLICY))
        return Availability::AccessDenied;
    if (result == HRESULT_FROM_WIN32(ERROR_BUSY) || result == HRESULT_FROM_WIN32(ERROR_SHARING_VIOLATION))
        return Availability::Busy;
    if (result == MF_E_VIDEO_RECORDING_DEVICE_INVALIDATED || result == HRESULT_FROM_WIN32(ERROR_NOT_FOUND))
        return Availability::Missing;
    if (result == E_NOTIMPL || result == MF_E_INVALIDMEDIATYPE)
...
{
    _confirmed.availability = Availability::Ready;
    _confirmed.revision = 1;
}
...
{
    if (!changed || !_capture || _identity.ownerSid.empty())
        return E_INVALIDARG;
    RETURN_IF_WIN32_BOOL_FALSE(DuplicateHandle(GetCurrentProcess(), changed, GetCurrentProcess(), _changed.put(),
...
{
    const auto lock = wil::AcquireSRWLockShared(&_lock);
    return _desiredEnabled || _confirmed.enabled || _request != _done;
}
...
HRESULT CameraController::Enable(bool enabled, uint64_t expectedRevision) noexcept
{
    return Change(nullptr, enabled, expectedRevision);
}
HRESULT CameraController::Change(const DeviceId* source, bool enabled, uint64_t expectedRevision) noexcept
{
    if (!_completed || !_changed)
        return E_UNEXPECTED;
    uint64_t serial = 0;
    ...
    DeviceId id;
    {
        ...
        if (_request != _done)
            return E_PENDING;
        if (expectedRevision != _confirmed.revision)
            return HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH);
        if (source)
            _desiredId = *source;
        else
            _desiredEnabled = enabled;
        if (_desiredEnabled && _desiredId.View().empty())
        {
    ...
    if (!_running && !desiredEnabled)
    {
        Finish(serial, id, false, S_OK);
        return S_OK;
    }
    if (!_running)
    {
    ...
    for (;;)
    {
        ...
        if (now >= deadline)
            return HRESULT_FROM_WIN32(
                ERROR_TIMEOUT); // Parent contains a stalled driver by terminating only this helper.
...
void CameraController::Finish(uint64_t serial, const DeviceId& id, bool enabled, HRESULT result) noexcept
{
    ...
    {
        _confirmed.sourceId = id;
        _confirmed.enabled = enabled;
        _confirmed.availability = availability;
        ++_confirmed.revision;
        SetEvent(_changed.get());
    }
    if (serial == _request && _done != serial)
    {
        _done = serial;
        _result = result;
        SetEvent(_completed.get());
    }
}
...
void CameraController::OnBridgeWork() noexcept
{
    ...
    if (!_capturing || _capturedId != id)
    {
        ...
        {
            if (!_frame)
                _frame = std::make_unique<std::array<BYTE, FrameBytes>>();
        }
        catch (const std::bad_alloc&)
        {
            Fail(serial, id, E_OUTOFMEMORY);
            return;
        }
        BeginDriverCall();
    ...
    if (frame == S_OK)
    {
        ...
        if (FAILED(published))
        {
            Fail(serial, id, published);
            return;
        }
        Finish(serial, id, true, S_OK);
    }
    else if (_published && pending)
        Finish(serial, id, true, S_OK);
}
