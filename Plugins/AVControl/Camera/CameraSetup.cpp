...
bool NativeArchitecture() noexcept
{
    ...
    if (!IsWow64Process2(GetCurrentProcess(), &process, &native) || process != IMAGE_FILE_MACHINE_UNKNOWN) return false;
#if defined(_M_ARM64)
...
int Run(int argc, wchar_t** argv)
{
    ...
    if (!NativeArchitecture()) { std::fputs("Use the native x64 or ARM64 setup program.\n", stderr); return 3; }
    ...
    if (!OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, token.put())) return 3;
    TOKEN_ELEVATION elevation{}; DWORD bytes = 0;
    if (!GetTokenInformation(token.get(), TokenElevation, &elevation, sizeof(elevation), &bytes) || elevation.TokenIsElevated)
...
int wmain(int argc, wchar_t** argv)
{
    ...
    catch (...) { std::fputs("Camera setup failed.\n", stderr); return 4; }
}
