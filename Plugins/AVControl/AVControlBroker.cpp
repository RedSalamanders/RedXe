...
void Broker::Stop(uint32_t drainMilliseconds) noexcept
{
    ...
    _job.reset();
    if (_process) (void)WaitForSingleObject(_process.get(), drainMilliseconds);
    _process.reset(); _shared.reset(); _mapping.reset(); _request.reset(); _reply.reset();
}
HRESULT Broker::Start(std::wstring_view executable, bool synthetic) noexcept
{
    Stop();
    bool successful = false;
    const auto rollback = wil::scope_exit([&] { if (!successful) Stop(); });
    try
    {
        if (executable.empty() || executable.find(L'"') != std::wstring_view::npos || executable.find(L'\0') != std::wstring_view::npos)
            return E_INVALIDARG;
        SECURITY_ATTRIBUTES security{sizeof(SECURITY_ATTRIBUTES), nullptr, TRUE};
        _mapping.reset(CreateFileMappingW(INVALID_HANDLE_VALUE, &security, PAGE_READWRITE, 0, sizeof(BrokerShared), nullptr));
        RETURN_LAST_ERROR_IF(!_mapping);
        _shared.reset(static_cast<BrokerShared*>(MapViewOfFile(_mapping.get(), FILE_MAP_ALL_ACCESS, 0, 0, sizeof(BrokerShared))));
        ...
        if (!_changed) _changed.reset(CreateEventW(&security, FALSE, FALSE, nullptr));
        if (_changed) _changed.ResetEvent();
        ...
        RETURN_IF_WIN32_BOOL_FALSE(SetInformationJobObject(_job.get(), JobObjectExtendedLimitInformation, &limits, sizeof(limits)));
        ...
        RETURN_IF_WIN32_BOOL_FALSE(UpdateProcThreadAttribute(startup.lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_HANDLE_LIST,
        ...
        if (ResumeThread(thread.get()) == DWORD(-1)) { const HRESULT hr = HRESULT_FROM_WIN32(GetLastError()); Stop(); return hr; }
        successful = true;
        return S_OK;
    }
...
{
    if (!_shared || !Running()) return HRESULT_FROM_WIN32(ERROR_PROCESS_ABORTED);
    if (!ValidInventoryPreferences(preferences)) return E_INVALIDARG;
...
{
    if (!cancelEvent || !timeoutMilliseconds || timeoutMilliseconds > 3000) return E_INVALIDARG;
    const auto started = GetTickCount64();
    if (!Running() || !_shared) return HRESULT_FROM_WIN32(ERROR_PROCESS_ABORTED);
    ...
    const DWORD wait = WaitForMultipleObjects(3, events, FALSE, timeoutMilliseconds > 250 ? timeoutMilliseconds - 250 : 1);
    if (wait != WAIT_OBJECT_0 + 2)
    {
        const HRESULT hr = HRESULT_FROM_WIN32(wait == WAIT_OBJECT_0 ? ERROR_CANCELLED :
            wait == WAIT_OBJECT_0 + 1 ? ERROR_PROCESS_ABORTED : wait == WAIT_TIMEOUT ? ERROR_TIMEOUT : GetLastError());
        const auto elapsed = GetTickCount64() - started;
        Stop(elapsed < timeoutMilliseconds ? static_cast<uint32_t>((std::min)(uint64_t{250}, timeoutMilliseconds - elapsed)) : 0);
        return hr;
    }
    MemoryBarrier();
    if (_shared->magic != BrokerMagic || _shared->version != BrokerProtocolVersion || _shared->sizeBytes != sizeof(BrokerShared) ||
        _shared->replySequence != _sequence || _shared->reply.result == E_PENDING || !ValidInventoryPayload(_shared->reply.inventory))
    {
        Stop(); return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    }
    reply = _shared->reply;
