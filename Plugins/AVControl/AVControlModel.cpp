...
bool ValidUtf8(std::string_view value, size_t maximumScalars, bool rejectControls) noexcept
{
    ...
    for (size_t i = 0; i < value.size();)
    {
        ...
        else if (first >= 0xE0 && first <= 0xEF) { trailing = 2; scalar = first & 0x0F; minimum = 0x800; }
        else if (first >= 0xF0 && first <= 0xF4) { trailing = 3; scalar = first & 0x07; minimum = 0x10000; }
        else if (first >= 0x80) return false;
        if (trailing > value.size() - i) return false;
        while (trailing--)
        {
        ...
        if (scalar < minimum || scalar > 0x10FFFF || (scalar >= 0xD800 && scalar <= 0xDFFF) || scalar == 0)
            return false;
        if (rejectControls && (scalar < 0x20 || (scalar >= 0x7F && scalar <= 0x9F))) return false;
...
{
    if (!ValidIdentifier(profile.id.View())) return ConfigurationError::Identifier;
    if (!ValidUtf8(profile.name.View(), MaximumNameScalars, true)) return ConfigurationError::Name;
    for (const DeviceId* id : {&profile.outputId, &profile.microphoneId, &profile.cameraId})
        if (!ValidUtf8(id->View(), MaximumDeviceIdBytes, false)) return ConfigurationError::DeviceIdentifier;
    if (profile.outputLevel > 100 || profile.microphoneLevel > 100) return ConfigurationError::Level;
    if (profile.audioRoles != AudioRoles::All && profile.audioRoles != AudioRoles::Communications) return ConfigurationError::Roles;
...
bool SameProfileId(std::string_view left, std::string_view right) noexcept
{
    ...
    const auto fold = [](char ch) noexcept { return ch >= 'A' && ch <= 'Z' ? static_cast<char>(ch + ('a' - 'A')) : ch; };
...
{
    if (error) *error = ConfigurationError::None;
    if (json.empty() || json.size() > MaximumSettingsBytes) return Fail(ConfigurationError::Capacity, error);
    JsonDocument document{yyjson_read(json.data(), json.size(), 0)};
    if (!document) return Fail(ConfigurationError::Json, error);
    yyjson_val* root = yyjson_doc_get_root(document.get());
    if (!yyjson_is_obj(root) || yyjson_obj_size(root) != 1) return Fail(ConfigurationError::Shape, error);
    yyjson_val* profiles = yyjson_obj_get(root, "profiles");
    if (!yyjson_is_arr(profiles)) return Fail(ConfigurationError::Shape, error);
    if (yyjson_arr_size(profiles) > MaximumProfiles) return Fail(ConfigurationError::Capacity, error);
    Configuration staged;
    size_t index = 0, maximum = 0;
    yyjson_val* value = nullptr;
    yyjson_arr_foreach(profiles, index, maximum, value)
    {
        if (!yyjson_is_obj(value) || yyjson_obj_size(value) != 9) return Fail(ConfigurationError::Shape, error);
        constexpr std::array keys{"id", "name", "outputId", "microphoneId", "cameraId", "audioRoles", "restoreLevels", "outputLevel", "microphoneLevel"};
        uint32_t seen = 0;
        ...
        yyjson_obj_foreach(value, memberIndex, memberCount, key, member)
        {
            ...
            if (found == keys.end()) return Fail(ConfigurationError::Shape, error);
            const uint32_t mask = 1U << static_cast<uint32_t>(found - keys.begin());
            if ((seen & mask) != 0) return Fail(ConfigurationError::Shape, error);
            seen |= mask;
        }
        Profile& profile = staged.profiles[index];
        if (!profile.id.Assign(String(yyjson_obj_get(value, "id")))) return Fail(ConfigurationError::Identifier, error);
        if (!profile.name.Assign(String(yyjson_obj_get(value, "name")))) return Fail(ConfigurationError::Name, error);
        if (!profile.outputId.Assign(String(yyjson_obj_get(value, "outputId"))) ||
            ...
            !profile.cameraId.Assign(String(yyjson_obj_get(value, "cameraId")))) return Fail(ConfigurationError::DeviceIdentifier, error);
        const std::string_view roles = String(yyjson_obj_get(value, "audioRoles"));
        if (roles != "all" && roles != "communications") return Fail(ConfigurationError::Roles, error);
        ...
        if (!yyjson_is_bool(restore)) return Fail(ConfigurationError::Shape, error);
        if (!yyjson_is_uint(outputLevel) || !yyjson_is_uint(inputLevel) || yyjson_get_uint(outputLevel) > 100 || yyjson_get_uint(inputLevel) > 100)
        ...
        if (validation != ConfigurationError::None) return Fail(validation, error);
        for (size_t previous = 0; previous < index; ++previous)
            if (SameProfileId(profile.id.View(), staged.profiles[previous].id.View())) return Fail(ConfigurationError::DuplicateProfile, error);
        ++staged.count;
    }
    destination = staged;
    return S_OK;
...
HRESULT SerializeConfiguration(const Configuration& configuration, char* destination, size_t capacity, uint32_t& writtenBytes) noexcept
{
    writtenBytes = 0;
    if (!destination) return E_POINTER;
    if (configuration.count > MaximumProfiles) return E_INVALIDARG;
    ...
    if (!root || !profiles || !yyjson_mut_obj_add_val(document.get(), root, "profiles", profiles)) return E_OUTOFMEMORY;
    ...
    for (size_t i = 0; i < configuration.count; ++i)
    {
        ...
        if (ValidateProfile(profile) != ConfigurationError::None) return E_INVALIDARG;
        for (size_t j = 0; j < i; ++j)
            if (SameProfileId(profile.id.View(), configuration.profiles[j].id.View())) return E_INVALIDARG;
        yyjson_mut_val* object = yyjson_mut_obj(document.get());
        if (!object || !yyjson_mut_arr_append(profiles, object)) return E_OUTOFMEMORY;
        ...
        if (!addString("id", profile.id.View()) || !addString("name", profile.name.View()) || !addString("outputId", profile.outputId.View()) ||
            !addString("microphoneId", profile.microphoneId.View()) || !addString("cameraId", profile.cameraId.View()) ||
            ...
            !yyjson_mut_obj_add_uint(document.get(), object, "microphoneLevel", profile.microphoneLevel)) return E_OUTOFMEMORY;
    }
    size_t bytes = 0;
    JsonBuffer json{yyjson_mut_write(document.get(), 0, &bytes)};
    if (!json) return E_OUTOFMEMORY;
    if (bytes > MaximumSettingsBytes || bytes > capacity) return HRESULT_FROM_WIN32(ERROR_INSUFFICIENT_BUFFER);
...
ProfileMatch MatchProfile(const Profile& profile, const ConfirmedState& state) noexcept
{
    if (state.output.availability != Availability::Ready || state.microphone.availability != Availability::Ready ||
        state.camera.availability != Availability::Ready || !(state.camera.sourceId == profile.cameraId)) return ProfileMatch::Custom;
    const size_t firstRole = profile.audioRoles == AudioRoles::Communications ? 2 : 0;
    for (size_t role = firstRole; role < 3; ++role)
        if (!(state.outputDefaults[role] == profile.outputId) || !(state.inputDefaults[role] == profile.microphoneId)) return ProfileMatch::Custom;
    if (!(state.output.id == profile.outputId) || !(state.microphone.id == profile.microphoneId)) return ProfileMatch::Custom;
    if (profile.restoreLevels && (profile.outputLevel != state.output.level || profile.microphoneLevel != state.microphone.level))
...
{
    Cancel();
    if (endpoint.availability != Availability::Ready || endpoint.id.View().empty() || endpoint.level > 100) return false;
    _id = endpoint.id;
...
{
    if (!_active || level > 100) return false;
    _draft = level;
    return true;
}
bool LevelGesture::Commit(const Endpoint& endpoint, uint64_t layoutRevision, uint32_t& level) noexcept
{
    const bool coherent = _active && endpoint.availability == Availability::Ready && endpoint.id == _id &&
        endpoint.generation == _generation && endpoint.levelRevision == _levelRevision && layoutRevision == _layoutRevision;
    const uint32_t draft = _draft;
    Cancel();
    if (coherent) level = draft;
    return coherent;
}
void LevelGesture::Cancel() noexcept { _active = false; _draft = _initial; }
uint32_t StepLevel(uint32_t level, int32_t delta) noexcept
{
    return static_cast<uint32_t>(std::clamp(static_cast<int64_t>(std::min(level, 100U)) + delta, int64_t{0}, int64_t{100}));
}
} // namespace AVControl
