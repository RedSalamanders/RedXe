...
template <size_t Capacity> struct DevicePreferences final
{
    ...
    {
        for (uint32_t i = 0; i < count; ++i) if (ids[i] == id) return i;
        return UnreferencedDevice;
    }
    bool Add(const DeviceId& id) noexcept
    {
        if (!id.length || Rank(id) != UnreferencedDevice) return true;
...
struct InventoryPreferences final
{
    ...
    void Add(const Profile& profile) noexcept
    {
        ...
        if (!output || !input || !camera) truncated = 1;
    }
...
template <typename Descriptor, size_t Capacity> class RankedInventory final
{
  public:
    ...
    {
        for (uint32_t i = 0; i < _count; ++i) if (_rows[i].id == row.id) return;
        if (_count < Capacity) { _ranks[_count] = rank; _rows[_count++] = row; return; }
        _truncated = true;
        uint32_t worst = 0;
        for (uint32_t i = 1; i < _count; ++i) if (_ranks[i] >= _ranks[worst]) worst = i;
        if (rank < _ranks[worst]) { _ranks[worst] = rank; _rows[worst] = row; }
    }
...
{
    for (uint32_t i = 0; i < defaults.size(); ++i) if (defaults[i] == id) return i;
