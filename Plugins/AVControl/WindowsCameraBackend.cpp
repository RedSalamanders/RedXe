...
bool EncodeCameraId(PCWSTR text, DeviceId& id) noexcept
{
    ...
    if (!length || length > MaximumDeviceIdBytes)
        return false;
...
class PhysicalCapture final : public Camera::CaptureSession
{
  public:
    ...
    {
        return _reader.ReadFrame(output, timestamp);
    }
...
struct WindowsCameraBackend::State final
{
    ...
    static DWORD CALLBACK Changed(HCMNOTIFICATION, void* context, CM_NOTIFY_ACTION action, PCM_NOTIFY_EVENT_DATA data,
                                  DWORD size) noexcept
    {
        ...
        if (data && size > offset && data->FilterType == CM_NOTIFY_FILTER_TYPE_DEVICEINTERFACE)
        {
            ...
            if (length < maximum)
            {
                ...
                if (self.selectedName[0] && CompareStringOrdinal(name, static_cast<int>(length),
                                                                 self.selectedName.data(), -1, TRUE) == CSTR_EQUAL)
                    ++self.selectedChanges;
            }
        }
        self.dirty.store(true, std::memory_order_release);
        if (self.inventoryChanged)
            SetEvent(self.inventoryChanged.get());
        if (self.changed)
    ...
    HRESULT Refresh() noexcept
    {
        ...
        const auto release = wil::scope_exit(
            [&]
            {
                ...
                {
                    wil::com_ptr_nothrow<IMFActivate> owner;
                    owner.attach(raw[i]);
                }
            });
        std::array<CameraDescriptor, MaximumCameras> next{};
        uint32_t admitted = 0;
        bool overflow = false;
        ...
        for (UINT32 i = 0; i < available; ++i)
        {
            wil::unique_cotaskmem_string symbolic, friendly;
            UINT32 length = 0;
            if (FAILED(raw[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_SOURCE_TYPE_VIDCAP_SYMBOLIC_LINK,
                                                  symbolic.put(), &length)))
                continue;
            if (!length || length > MaximumDeviceIdBytes)
            {
            ...
            if (!route.SymbolicLink().empty() &&
                CompareStringOrdinal(symbolic.get(), static_cast<int>(length), route.SymbolicLink().data(),
                                     static_cast<int>(route.SymbolicLink().size()), TRUE) == CSTR_EQUAL)
                continue;
            // Never wrap this route or another identified software camera into a recursive capture pipeline.
            DEVPROP_BOOLEAN virtualDevice = DEVPROP_FALSE;
            DEVPROPTYPE type = 0;
            ULONG bytes = sizeof(virtualDevice);
            if (CM_Get_Device_Interface_PropertyW(symbolic.get(), &DEVPKEY_DeviceInterface_IsVirtualCamera, &type,
                                                  reinterpret_cast<PBYTE>(&virtualDevice), &bytes, 0) == CR_SUCCESS &&
                type == DEVPROP_TYPE_BOOLEAN && virtualDevice != DEVPROP_FALSE)
                continue;
            CameraDescriptor device;
            if (!EncodeCameraId(symbolic.get(), device.id))
            {
                overflow = true;
                continue;
            }
            ...
            if (SUCCEEDED(raw[i]->GetAllocatedString(MF_DEVSOURCE_ATTRIBUTE_FRIENDLY_NAME, friendly.put(), &length)))
            {
                ...
                if (copy && friendly.get()[copy - 1] >= 0xd800 && friendly.get()[copy - 1] <= 0xdbff)
                    --copy;
        ...
        if (controller)
        {
            ...
            if (state.sourceId.View().empty() && count)
                RETURN_IF_FAILED(controller->Select(cameras[0].id, state.revision));
    ...
    void Copy(Inventory& inventory) noexcept
    {
        ...
        inventory.cameraCount = count;
        inventory.truncated = inventory.truncated || truncated;
        ...
        if (controller)
        {
            ...
            if (!Present(inventory.state.camera.sourceId))
                inventory.state.camera.availability = Availability::Missing;
        }
        else
        {
            inventory.state.camera = {};
            inventory.state.camera.availability =
                routeResult == E_ACCESSDENIED ? Availability::AccessDenied : Availability::Unsupported;
        }
    }
};
...
{
    if (_state->preferences == preferences)
        return;
...
HRESULT WindowsCameraBackend::Initialize(HANDLE changed) noexcept
{
    ...
    if (!_state->inventoryChanged)
        return HRESULT_FROM_WIN32(GetLastError());
    RETURN_IF_FAILED(MFStartup(MF_VERSION));
    _state->startedMf = true;
    ...
    if (SUCCEEDED(_state->routeResult))
    {
        ...
        {
            _state->controller =
                std::make_unique<Camera::CameraController>(std::move(identity), std::make_unique<PhysicalCapture>());
        }
    ...
    const CONFIGRET registered =
        CM_Register_Notification(&filter, _state.get(), State::Changed, _state->notification.put());
    if (registered != CR_SUCCESS)
        return HRESULT_FROM_WIN32(CM_MapCrToWin32Err(registered, ERROR_GEN_FAILURE));
    return _state->Refresh();
}
HRESULT WindowsCameraBackend::Observe(Inventory& inventory) noexcept
{
    const HRESULT result = _state->Refresh();
    _state->Copy(inventory);
    if (FAILED(result))
        inventory.state.camera.availability = Availability::Unknown;
    return result;
}
HRESULT WindowsCameraBackend::Execute(const BrokerCommand& command, Inventory& inventory) noexcept
{
    ...
    if (SUCCEEDED(result))
    {
        if (!_state->controller)
            result = FAILED(_state->routeResult) ? _state->routeResult : E_NOTIMPL;
        else if (command.device != DeviceKind::Camera)
            result = E_INVALIDARG;
        else if (command.operation == BrokerOperation::SetCameraSource)
            result = _state->Present(command.id) ? _state->controller->Select(command.id, command.expectedRevision)
                                                 : HRESULT_FROM_WIN32(ERROR_NOT_FOUND);
        else if (command.operation == BrokerOperation::SetCameraEnabled && command.value <= 1)
            result = command.value && !_state->Present(_state->controller->Snapshot().sourceId)
                         ? HRESULT_FROM_WIN32(ERROR_NOT_FOUND)
    ...
    if (_state->controller)
        _state->Selected(_state->controller->Snapshot().sourceId);
    _state->Copy(inventory);
    if (!observed)
        inventory.state.camera.availability = Availability::Unknown;
    return result;
}
bool WindowsCameraBackend::RequiresHelper() noexcept
{
    return _state->controller && _state->controller->RequiresHelper();
}
HANDLE WindowsCameraBackend::ProgressEvent() const noexcept
{
    return _state->controller ? _state->controller->ProgressEvent() : nullptr;
}
HANDLE WindowsCameraBackend::NotificationEvent() const noexcept
{
    return _state->inventoryChanged.get();
}
HRESULT WindowsCameraBackend::ProcessNotifications() noexcept
{
    return _state->Refresh();
}
DWORD WindowsCameraBackend::WatchdogTimeout() const noexcept
{
    return _state->controller ? _state->controller->WatchdogTimeout() : INFINITE;
}
} // namespace AVControl
