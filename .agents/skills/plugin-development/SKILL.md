...
only when work concerns interfaces that the normative contract has not frozen yet, such as data providers,
...
- Keep `Widget.h` rendering-neutral. Generic widgets expose the frozen GPU or future mechanisms through distinct IIDs;
  never add a plugin's geometry, shader, command, or HWND concept to the generic root. GPU and native-window widgets
  use their independent frozen mechanism IIDs.
...
- Window widgets receive only a borrowed host-owned child container and own all children, timers, controllers, and GDI
  resources they create. They stop animation in `SetVisible(FALSE)` and destroy children before `Detach` returns.
