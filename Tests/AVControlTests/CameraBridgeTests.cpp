...
template<class Condition> void Until(Condition condition, const char* message)
{
    ...
    while (!condition() && GetTickCount64() < deadline) std::this_thread::sleep_for(std::chrono::milliseconds(2));
    Check(condition(), message);
}
std::wstring CurrentSid()
{
    wil::unique_handle token;
    Check(OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, token.put()) != FALSE, "bridge fixture reads its own identity");
    ...
    Check(ConvertSidToStringSidW(reinterpret_cast<TOKEN_USER*>(data.data())->User.Sid, sid.put()) != FALSE, "bridge fixture canonical SID");
    return sid.get();
}
struct Observer final : BridgeObserver
{
    std::atomic<uint32_t> calls{0}, stops{0};
    void OnBridgeWork() noexcept override { ++calls; }
    void OnBridgeStopping() noexcept override { ++stops; }
};
wil::unique_handle OpenPipe(const BridgeIdentity& identity)
{
    return wil::unique_handle(CreateFileW(identity.pipeName.c_str(), GENERIC_READ | GENERIC_WRITE | READ_CONTROL, 0, nullptr,
...
uint32_t RunCameraBridgeTests()
{
    ...
    const auto sid = CurrentSid(); GUID guid{};
    Check(SUCCEEDED(CoCreateGuid(&guid)), "bridge fixture unique namespace");
    BridgeIdentity identity;
    Check(MakeBridgeIdentity(sid.c_str(), identity, &guid) == S_OK && identity.synthetic, "bridge isolated admission is explicit");
    BridgeIdentity preserved = identity;
    Check(FAILED(MakeBridgeIdentity(L"bad)SID", identity)) && identity.pipeName == preserved.pipeName, "bridge rejects injected or invalid SID transactionally");
    Observer observer;
    BridgeServer server;
    ...
    Check(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime()) == S_OK && output[0] == 16,
        "a connected route starts neutral before camera-on"); provider.EndFrame();
    ...
    Check(server.DemandingConsumers() == 0 && server.Consumers() == 1, "paused sample demand expires without closing the selected camera stream");
    ...
    Check(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime()) == S_OK && output[0] == 235,
    ...
    HRESULT off = S_OK;
    std::thread offThread([&] { off = server.SetGate(3, false); }); offThread.join();
    Check(off == HRESULT_FROM_WIN32(ERROR_TIMEOUT), "camera off cannot acknowledge past an uncompleted media publication");
    provider.EndFrame();
    Check(server.Publish(2, frame, MFGetSystemTime()) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "old-source callback rejected after off intent");
    ...
    Check(FAILED(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime())),
        "disconnected provider requests full neutral replacement immediately"); provider.EndFrame();
    provider.Stop();
    ...
    Check(server.SetGate(4, true) == S_OK && server.Publish(4, frame, MFGetSystemTime()) == S_OK, "new generation restarts from a fresh image");
    Check(server.SetGate(5, false) == S_OK, "uncontended off is acknowledged");
    Check(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime()) == S_OK && output[0] == 16,
        "acknowledged off has no old physical pixels"); provider.EndFrame();
    Check(server.SetGate(6, true) == S_OK && server.Publish(6, frame, MFGetSystemTime()) == S_OK && server.SetGate(7, true) == S_OK,
        "source replacement changes generation while remaining on");
    Check(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime()) == S_OK && output[0] == 16,
        "source replacement clears the previous source until a new frame"); provider.EndFrame();
    Check(server.Publish(6, frame, MFGetSystemTime()) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "late replaced-source callback is rejected");
    provider.Stop(); Until([&] { return server.Consumers() == 0; }, "last consumer close releases its pipe and frame lease");
    ...
    BridgeHello hello; hello.version = 0; BridgeReply reply;
    auto deadline = GetTickCount64() + BridgeDeadlineMs;
    Check(BridgeTransfer(invalid.get(), true, &hello, sizeof(hello), deadline) == S_OK &&
        BridgeTransfer(invalid.get(), false, &reply, sizeof(reply), deadline) == S_OK && FAILED(reply.status), "wrong protocol is explicitly rejected");
    invalid.reset();
    ...
    Check(excess.Start() == S_OK && FAILED(excess.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime())),
        "excess consumer stays neutral with no unbounded slot allocation"); excess.EndFrame(); excess.Stop();
    ...
    server.Stop(); Check(observer.stops == 1, "bridge shutdown invokes capture cleanup once");
    server.Stop(); Check(observer.stops == 1, "bridge shutdown is idempotent");
    ...
    server.Stop();
    Check(FAILED(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime())),
        "helper death causes immediate neutral fallback"); provider.EndFrame(); provider.Stop();
    ...
    Check(server.Start(identity, nullptr) == S_OK, "fixture starts production principal admission in its isolated namespace");
    auto untrusted = OpenPipe(identity); Check(!!untrusted, "own user can reach admission for authorization check");
    hello = {}; deadline = GetTickCount64() + BridgeDeadlineMs;
    Check(BridgeTransfer(untrusted.get(), true, &hello, sizeof(hello), deadline) == S_OK &&
        BridgeTransfer(untrusted.get(), false, &reply, sizeof(reply), deadline) == S_OK && reply.status == E_ACCESSDENIED,
...
namespace
{
std::wstring ReadyName(PCWSTR guid) { return std::wstring(L"Local\\RedXe.Camera.Test.Ready.") + guid; }
struct ChildFrames final : BridgeObserver
{
    explicit ChildFrames(BridgeServer& value) : server(value), pixels(static_cast<size_t>(FrameWidth) * FrameHeight * 3 / 2, 128)
    {
    ...
    void OnBridgeWork() noexcept override
    {
        ...
        {
            admitted = true;
            if (FAILED(server.SetGate(2, true)) || FAILED(server.Publish(2, pixels, MFGetSystemTime()))) SetEvent(done.get());
        }
        else if (admitted && !server.Consumers()) SetEvent(done.get());
    }
...
int RunCameraBridgeChild(const wchar_t* identifier)
{
    using namespace AVControl::Camera;
    GUID guid{};
    if (!identifier || wcsnlen_s(identifier, 40) != 38 || FAILED(CLSIDFromString(identifier, &guid))) return 2;
    if (FAILED(CoInitializeEx(nullptr, COINIT_MULTITHREADED))) return 3;
    const auto com = wil::scope_exit([] { CoUninitialize(); });
    if (FAILED(MFStartup(MF_VERSION))) return 4;
    const auto mf = wil::scope_exit([] { (void)MFShutdown(); });
    BridgeIdentity identity;
    if (FAILED(MakeBridgeIdentity(CurrentSid().c_str(), identity, &guid))) return 5;
    BridgeServer server; ChildFrames observer(server);
    const auto stop = wil::scope_exit([&] { server.Stop(); });
    if (!observer.done || FAILED(server.Start(identity, &observer))) return 6;
    wil::unique_handle ready(OpenEventW(EVENT_MODIFY_STATE, FALSE, ReadyName(identifier).c_str()));
    if (!ready || !SetEvent(ready.get())) return 7;
    return WaitForSingleObject(observer.done.get(), 5000) == WAIT_OBJECT_0 && observer.admitted && server.Consumers() == 0 ? 0 : 8;
}
uint32_t RunCameraCrossProcessTests()
{
    ...
    const auto mf = wil::scope_exit([] { (void)MFShutdown(); });
    GUID guid{}; wchar_t identifier[40]{};
    Check(SUCCEEDED(CoCreateGuid(&guid)) && StringFromGUID2(guid, identifier, static_cast<int>(std::size(identifier))) != 0,
    ...
    Check(GetModuleFileNameW(nullptr, executable, static_cast<DWORD>(std::size(executable))) != 0, "bridge fixture locates its own child executable");
    ...
    Check(SetInformationJobObject(job.get(), JobObjectExtendedLimitInformation, &limits, sizeof(limits)) != FALSE, "fixture job contains exactly one owned child");
    ...
    Check(CreateProcessW(executable, command.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW | CREATE_SUSPENDED,
        nullptr, nullptr, &startup, &created) != FALSE, "fixture starts a separate camera helper without inherited handles");
    ...
    Check(WaitForSingleObject(ready.get(), 3000) == WAIT_OBJECT_0, "child bridge accepts connections within the startup deadline");
    BridgeIdentity identity;
    Check(MakeBridgeIdentity(CurrentSid().c_str(), identity, &guid) == S_OK, "parent uses the exact child fixture identity");
    BridgeFrameProvider provider(identity);
    ...
    bool bright = false; const auto deadline = GetTickCount64() + 1000;
    while (!bright && GetTickCount64() < deadline)
    {
        const HRESULT result = provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime());
        provider.EndFrame();
        bright = result == S_OK && output.front() == 210 && output[FrameWidth * FrameHeight - 1] == 210 && output.back() == 128;
        if (!bright) std::this_thread::sleep_for(std::chrono::milliseconds(2));
    }
    Check(bright, "a separate process transfers actual NV12 pixels through the admitted mapping");
    provider.Stop();
    Check(WaitForSingleObject(process.get(), 3000) == WAIT_OBJECT_0, "consumer disconnect releases the child acquisition lease");
    DWORD result = 99;
    Check(GetExitCodeProcess(process.get(), &result) != FALSE && result == 0, "cross-process camera fixture exits cleanly with no orphan helper");
    return checks;
}
...
int RunCameraWatchdogChild(const wchar_t* identifier, const wchar_t* stage)
{
    ...
    if (!identifier || wcsnlen_s(identifier, 40) != 38 || FAILED(CLSIDFromString(identifier, &guid)) ||
        !stage || stage[0] < L'0' || stage[0] > L'2' || stage[1]) return 2;
    if (FAILED(CoInitializeEx(nullptr, COINIT_MULTITHREADED))) return 3;
    const auto com = wil::scope_exit([] { CoUninitialize(); });
    if (FAILED(MFStartup(MF_VERSION))) return 4;
    const auto mf = wil::scope_exit([] { (void)MFShutdown(); });
    ...
    AVControl::DeviceId id; (void)id.Assign("isolated-hung-capture");
    if (!changed || FAILED(controller.Initialize(changed.get())) || FAILED(controller.Select(id, controller.Snapshot().revision)) ||
        FAILED(controller.Enable(true, controller.Snapshot().revision)) || controller.WatchdogTimeout() != INFINITE) return 6;
    wil::unique_handle ready(OpenEventW(EVENT_MODIFY_STATE, FALSE, ReadyName(identifier).c_str()));
    if (!ready || !SetEvent(ready.get())) return 7;
    ...
    for (;;)
    {
        ...
        { (void)TerminateProcess(GetCurrentProcess(), ERROR_TIMEOUT); return 9; }
        if (wait != WAIT_TIMEOUT) return 8;
    }
}
uint32_t RunCameraWatchdogTests()
{
    ...
    {
        GUID guid{}; wchar_t identifier[40]{}, executable[32768]{};
        Check(SUCCEEDED(CoCreateGuid(&guid)) && StringFromGUID2(guid, identifier, static_cast<int>(std::size(identifier))) != 0,
        ...
        Check(GetModuleFileNameW(nullptr, executable, static_cast<DWORD>(std::size(executable))) != 0, "watchdog locates its own fixture executable");
        std::wstring command = std::wstring(L"\"") + executable + L"\" --camera-watchdog-fixture " + identifier + L" " + std::to_wstring(stage);
        wil::unique_handle job(CreateJobObjectW(nullptr, nullptr));
        ...
        STARTUPINFOW startup{sizeof(startup)}; PROCESS_INFORMATION created{};
        Check(CreateProcessW(executable, command.data(), nullptr, nullptr, FALSE, CREATE_NO_WINDOW | CREATE_SUSPENDED,
            nullptr, nullptr, &startup, &created) != FALSE, "watchdog fixture child starts without inherited handles");
        wil::unique_handle process(created.hProcess), thread(created.hThread);
        if (!AssignProcessToJobObject(job.get(), process.get()))
        {
            (void)TerminateProcess(process.get(), ERROR_CANCELLED);
        ...
        Check(WaitForSingleObject(ready.get(), 3000) == WAIT_OBJECT_0, "watchdog child arms without opening capture or scheduling idle timeouts");
        BridgeIdentity identity;
        Check(MakeBridgeIdentity(CurrentSid().c_str(), identity, &guid) == S_OK, "watchdog parent uses the unique synthetic channel");
        BridgeFrameProvider provider(identity);
        ...
        Check(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime()) == S_OK,
            "watchdog fixture requests its first frame"); provider.EndFrame();
        if (stage == 2)
        {
            bool bright = false; const auto deadline = GetTickCount64() + 500;
            while (!bright && GetTickCount64() < deadline)
            {
                const HRESULT read = provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime());
                provider.EndFrame(); bright = read == S_OK && output.front() == 210;
                if (!bright) Sleep(2);
            }
        ...
        Check(FAILED(provider.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime())),
            "terminated producer cannot replay its cached image through a successful fill"); provider.EndFrame(); provider.Stop();
    }
    return checks;
}
