...
uint32_t RunBrokerTests()
{
    ...
    auto start = [&]
    {
        ...
        Check(broker.Execute({}, *reply, cancel.get(), 3000) == S_OK,
              "synthetic helper observes without hardware access");
        Check(reply->inventory.outputCount == 2 && reply->inventory.inputCount == 2 &&
                  reply->inventory.cameraCount == 2,
    ...
    Check(broker.SetPreferences(*preferences) == E_INVALIDARG,
          "malformed preference union rejected before shared publication");
    *preferences = {};
    preferences->truncated = 1;
    Check(broker.SetPreferences(*preferences) == S_OK && broker.Execute({}, *reply, cancel.get(), 3000) == S_OK &&
              reply->inventory.truncated,
    ...
    command.operation = BrokerOperation::SetLevel;
    command.device = DeviceKind::Microphone;
    command.id = reply->inventory.state.microphone.id;
    command.expectedGeneration = reply->inventory.state.microphone.generation;
    command.expectedRevision = reply->inventory.state.microphone.levelRevision;
    command.value = 37;
    ...
    Check(broker.Execute(command, *reply, cancel.get(), 3000) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH) &&
              reply->inventory.state.microphone.level == 37,
          "stale endpoint revision is rejected with actual state");
    const auto confirmedRevision = reply->inventory.state.revision;
    Check(broker.Execute({}, *reply, nullptr, 3000) == E_INVALIDARG &&
              broker.Execute({}, *reply, cancel.get(), 0) == E_INVALIDARG,
    ...
    command = {};
    command.operation = BrokerOperation::FixtureHang;
    const auto began = GetTickCount64();
    Check(broker.Execute(command, *reply, cancel.get(), 500) == HRESULT_FROM_WIN32(ERROR_TIMEOUT),
          "stalled device operation times out");
    Check(GetTickCount64() - began < 750 && !broker.Running(),
          "timeout kills only owned helper within bounded latency");
    ...
    start();
    {
        ...
        Check(broker.Execute(command, *reply, cancel.get(), 3000) == HRESULT_FROM_WIN32(ERROR_CANCELLED),
              "cancel interrupts stalled helper");
        Check(GetTickCount64() - when < 750 && !broker.Running(), "cancel drains owned child promptly");
    }
    start();
    command.operation = BrokerOperation::FixtureExit;
    Check(broker.ChangeEvent() == stableChangeEvent,
    ...
    {
        start();
        const auto preserved = reply->inventory.state.revision;
        command.operation = BrokerOperation::FixtureMalformedReply;
        command.value = corruption;
        Check(broker.Execute(command, *reply, cancel.get(), 3000) == HRESULT_FROM_WIN32(ERROR_INVALID_DATA) &&
                  !broker.Running(),
    ...
    broker.Stop();
    Check(!broker.Running() && WaitForSingleObject(process.get(), 500) == WAIT_OBJECT_0,
          "normal shutdown leaves no orphan helper");
