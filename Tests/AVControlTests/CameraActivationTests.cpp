...
uint32_t RunCameraActivationTests()
{
    ...
    Check(factoryEntry(GUID_NULL, IID_IClassFactory, &invalid) == CLASS_E_CLASSNOTAVAILABLE && !invalid,
          "camera DLL rejects unrelated COM class IDs");
    wil::com_ptr_nothrow<IClassFactory> factory;
    Check(factoryEntry(CameraSourceClsid, IID_PPV_ARGS(factory.put())) == S_OK && unloadEntry() == S_FALSE,
          "class factory pins DLL lifetime");
    ...
    Check(SUCCEEDED(activation.query_to(attributes.put())) && SUCCEEDED(activation.query_to(identity.put())) &&
              SUCCEEDED(attributes.query_to(attributeIdentity.put())) && identity == attributeIdentity,
          "activation and attributes share controlling COM identity");
    identity.reset();
    attributeIdentity.reset();
    wil::com_ptr_nothrow<IMFMediaSource> source;
    Check(FAILED(activation->ActivateObject(IID_PPV_ARGS(source.put()))) && !source,
          "missing owner does not activate an unbound route");
    Check(attributes->SetString(CameraOwnerSidAttribute, L"invalid sid") == S_OK &&
              FAILED(activation->ActivateObject(IID_PPV_ARGS(source.put()))),
    ...
    Check(attributes->SetString(CameraOwnerSidAttribute, L"S-1-5-21-1-2-3-1001") == S_OK &&
              activation->ActivateObject(IID_PPV_ARGS(source.put())) == S_OK,
          "valid activation constructs metadata lazily");
    const auto cleanupSource = wil::scope_exit(
        [&]
        {
            if (source)
    ...
    Check(activation->ActivateObject(IID_PPV_ARGS(same.put())) == S_OK && source == same,
          "repeated activation returns the same live object");
    same.reset();
    wil::com_ptr_nothrow<IMFPresentationDescriptor> presentation;
    Check(source->CreatePresentationDescriptor(presentation.put()) == S_OK,
          "activated DLL exposes real media metadata");
    presentation.reset();
    Check(activation->DetachObject() == S_OK && source->CreatePresentationDescriptor(presentation.put()) == S_OK,
          "detach transfers shutdown responsibility without stopping the caller's source");
    presentation.reset();
    Check(activation->ShutdownObject() == S_OK, "empty activation shutdown is idempotent");
    attributes.reset();
    activation.reset();
    Check(factory->LockServer(FALSE) == S_OK, "camera COM server lock drains");
    factory.reset();
    Check(unloadEntry() == S_FALSE, "detached source alone still pins camera DLL");
    Check(source->Shutdown() == S_OK && source->Shutdown() == S_OK, "detached source has idempotent explicit shutdown");
    source.reset();
    Check(unloadEntry() == S_OK, "all detached source/stream references drain before DLL unload");

    Check(factoryEntry(CameraSourceClsid, IID_PPV_ARGS(factory.put())) == S_OK &&
              factory->CreateInstance(nullptr, IID_PPV_ARGS(activation.put())) == S_OK,
          "camera activation is reusable");
    Check(activation->SetString(CameraOwnerSidAttribute, L"S-1-5-21-1-2-3-1001") == S_OK &&
              activation->ActivateObject(IID_PPV_ARGS(source.put())) == S_OK,
          "second metadata activation");
    Check(activation->ShutdownObject() == S_OK &&
              source->CreatePresentationDescriptor(presentation.put()) == MF_E_SHUTDOWN,
