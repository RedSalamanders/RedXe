...
class Host final : public RedXeComObject<Host, IRedXeHost>
{
  public:
    ...
    HRESULT STDMETHODCALLTYPE GetDataProvider(const char*, IRedXeDataProvider** value) noexcept override
        { if (value) *value = nullptr; return E_NOINTERFACE; }
    HRESULT STDMETHODCALLTYPE RequestFrame() noexcept override { ++frames; return S_OK; }
    HRESULT STDMETHODCALLTYPE ReportWidgetStatus(const char*, const RedXeWidgetStatusReport*) noexcept override { return S_OK; }
    HRESULT STDMETHODCALLTYPE PersistWidgetSettings(const char*, const char*, uint32_t) noexcept override { return E_NOTIMPL; }
    HRESULT STDMETHODCALLTYPE Log(const RedXeLogRecord*) noexcept override { return S_OK; }
    HRESULT STDMETHODCALLTYPE QueueControlWork(IRedXeControlWork* work) noexcept override
    {
        if (reject) return E_ACCESSDENIED;
        if (pending) return HRESULT_FROM_WIN32(ERROR_BUSY);
        pending = work;
        return S_OK;
    }
    void RunNext()
    ...
    void Drain()
    {
        for (uint32_t i = 0; pending && i < 12; ++i) RunNext();
...
{
    return {AVControl::ViewCommandKind::SetMuted, kind, endpoint.id, endpoint.generation, endpoint.muteRevision, muted ? 1U : 0U};
}
}
uint32_t RunCoordinatorTests()
{
    ...
    host.Drain();
    Check(host.runs == 1 && coordinator->Current().outputCount == 2, "shared observation publishes confirmed synthetic inventory");
    const auto initialDeviceRevision = coordinator->DeviceRevision();
    const auto output = coordinator->Current().state.output;
    ViewCommand level{ViewCommandKind::SetLevel, DeviceKind::Output, output.id, output.generation, output.levelRevision, 26};
    ...
    Check(SUCCEEDED(coordinator->Submit(Mute(output, DeviceKind::Output, true))), "output mute accepted beside pending level");
    host.RunNext();
    ...
    Check(coordinator->DeviceRevision() == initialDeviceRevision, "level-only acknowledgement does not rebuild device selectors");
    ...
    host.Drain();
    Check(coordinator->LastResult() == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH) && coordinator->Current().state.output.level == 39,
    ...
    Check(profile.id.Assign("studio") && profile.name.Assign("Studio") &&
        ...
        profile.cameraId.Assign(coordinator->Current().cameras[1].id.View()), "profile fixture binds second devices");
    ...
    Check(coordinator->SubmitProfile(profile) == HRESULT_FROM_WIN32(ERROR_BUSY), "concurrent profile operation rejected");
    Check(coordinator->Submit(level) == HRESULT_FROM_WIN32(ERROR_BUSY), "volume from the old route is disabled during profile change");
    ...
    host.Drain();
    Check(coordinator->LastApply().applied && !coordinator->ProfilePending(), "profile completes and publishes transaction outcome");
    Check(MatchProfile(profile, coordinator->Current().state) == ProfileMatch::Active, "profile identity waits for complete observed routing");
    Check(coordinator->Current().state.output.muted && coordinator->Current().state.microphone.muted && !coordinator->Current().state.camera.enabled,
    ...
    Check(SUCCEEDED(coordinator->Submit({ViewCommandKind::SetCameraEnabled, DeviceKind::Camera, camera.sourceId, 0, camera.revision, 1})),
    ...
    for (uint32_t i = 0; i < 100; ++i) coordinator->Prepare();
    Check(!host.pending && host.frames == hiddenFrames, "hidden coordinator produces no observations or frame loop");
    coordinator->SetSubscriberVisible(true);
    coordinator->Prepare();
    host.Drain();
    Check(coordinator->Current().state.output.availability == Availability::Ready, "showing reconnects an owned stopped helper");
    ...
    Check(SUCCEEDED(coordinator->Submit({ViewCommandKind::SetCameraEnabled, DeviceKind::Camera, camera.sourceId, 0, camera.revision, 0})),
        ...
        if (i < 9)
        {
            ...
            for (uint32_t j = 0; j < MaximumProfiles; ++j)
            {
                ...
                Check(item.outputId.Assign("retained-output-" + std::to_string(i * MaximumProfiles + j)) &&
                    ...
                    item.cameraId.Assign("retained-camera-" + std::to_string(i * MaximumProfiles + j)), "independent saved binding fixture");
            }
        }
        Check(coordinator->RegisterConfiguration(&configuration) == (i < 64 ? S_OK : HRESULT_FROM_WIN32(ERROR_NOT_ENOUGH_QUOTA)),
    ...
    coordinator->SetSubscriberVisible(true); coordinator->Prepare(); host.Drain();
    Check(coordinator->Current().truncated, "all loaded widget bindings contribute to the shared bounded inventory union");
    const auto membershipRevision = coordinator->DeviceRevision();
    for (size_t i = 4; i < configurations->size(); ++i) coordinator->UnregisterConfiguration(&(*configurations)[i]);
    coordinator->Prepare(); host.Drain();
    ...
    coordinator->ConfigurationChanged(); coordinator->Prepare(); host.Drain();
    Check(!coordinator->Current().truncated, "identical references from another profile do not consume duplicate capacity");
    ...
    coordinator->SetSubscriberVisible(false); host.Drain();
    for (const auto& configuration : *configurations) coordinator->UnregisterConfiguration(&configuration);
