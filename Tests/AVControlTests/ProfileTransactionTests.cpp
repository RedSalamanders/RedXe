...
class Intercept final : public AVControl::BackendTransport
{
  public:
    ...
    HRESULT Execute(const AVControl::BrokerCommand& command, AVControl::BrokerReply& reply, HANDLE cancel, uint32_t timeout) noexcept override
    {
        ...
        if (command.operation != BrokerOperation::Observe && ++mutations == failAt)
        {
            if (externalLevelChange)
            {
                ...
                BrokerCommand external{BrokerOperation::SetLevel, DeviceKind::Output, endpoint.id, 91, 0, endpoint.generation, endpoint.levelRevision};
        ...
        if (SUCCEEDED(hr) && removeCameraCapability) reply.inventory.capabilities &= ~CapabilityCameraRoute;
...
uint32_t RunProfileTransactionTests()
{
    ...
    Profile profile;
    Check(profile.id.Assign("meeting") && profile.name.Assign("Meeting") && profile.outputId.Assign("fixture-output-2") &&
          profile.microphoneId.Assign("fixture-input-2") && profile.cameraId.Assign("fixture-camera-2"), "profile fixture IDs");
    profile.restoreLevels = true; profile.outputLevel = 48; profile.microphoneLevel = 61;
    ...
    auto start = [&] {
        ...
        backend.removeCameraCapability = backend.externalLevelChange = false; backend.safety = nullptr;
    ...
    Check(applied.applied && applied.result == S_OK && applied.mutations == 10, "profile applies every required mutation");
    for (size_t role = 0; role < 3; ++role)
        Check(reply->inventory.state.outputDefaults[role] == profile.outputId && reply->inventory.state.inputDefaults[role] == profile.microphoneId,
            "profile verifies every render/capture role");
    Check(reply->inventory.state.microphone.muted && !reply->inventory.state.camera.enabled, "profile preserves mic mute and camera off across source changes");
    Check(reply->inventory.state.camera.sourceId == profile.cameraId && reply->inventory.state.output.level == 48 &&
        reply->inventory.state.microphone.level == 61, "profile restores optional levels and camera binding");
    backend.mutations = 0;
    ...
    {
        start(); backend.failAt = failure;
        const auto result = ApplyProfile(profile, backend, *reply, safety, cancel.get(), 3000);
        Check(result.result == E_FAIL && !result.applied && result.rollbackComplete, "each failed stage rolls back owned earlier fields");
        for (size_t role = 0; role < 3; ++role)
            Check(reply->inventory.state.outputDefaults[role] == reply->inventory.outputs[0].id &&
                  reply->inventory.state.inputDefaults[role] == reply->inventory.inputs[0].id, "failed profile restores original audio roles");
        Check(reply->inventory.outputs[1].level == 65 && reply->inventory.inputs[1].level == 72 && !reply->inventory.inputs[1].muted,
            "failed profile restores destination levels and transaction-owned mute");
        Check(reply->inventory.state.camera.sourceId == reply->inventory.cameras[0].id && !reply->inventory.state.camera.enabled,
            "failed profile never enables camera");
    }
    start(); backend.removeCameraCapability = true;
    ...
    start();
    Profile missing = profile; Check(missing.cameraId.Assign("absent-camera"), "missing camera fixture");
    result = ApplyProfile(missing, backend, *reply, safety, cancel.get(), 3000);
    Check(result.result == HRESULT_FROM_WIN32(ERROR_NOT_FOUND) && backend.mutations == 0, "missing binding prevents all mutations");
    start(); backend.failAt = 5; backend.externalLevelChange = true;
    ...
    start(); backend.safety = &safety; backend.safetyAt = 4;
    result = ApplyProfile(profile, backend, *reply, safety, cancel.get(), 3000);
    ...
    start(); profile.audioRoles = AudioRoles::Communications;
    result = ApplyProfile(profile, backend, *reply, safety, cancel.get(), 3000);
    Check(result.applied && reply->inventory.state.outputDefaults[2] == profile.outputId && reply->inventory.state.inputDefaults[2] == profile.microphoneId,
        "communications profile applies its exact role");
    for (size_t role = 0; role < 2; ++role)
        Check(reply->inventory.state.outputDefaults[role] == reply->inventory.outputs[0].id &&
              reply->inventory.state.inputDefaults[role] == reply->inventory.inputs[0].id, "communications profile preserves other roles");
    backend.broker.Stop();
