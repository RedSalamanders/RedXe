...
class Frames final : public FrameProvider
{
  public:
    ...
    {
        ++fills;
        if (pitch < static_cast<LONG>(FrameWidth) || static_cast<uint64_t>(pitch) * FrameHeight * 3 / 2 > capacity) return E_INVALIDARG;
        ...
        std::memset(bytes + static_cast<size_t>(pitch) * FrameHeight, 100, static_cast<size_t>(pitch) * FrameHeight / 2);
...
wil::com_ptr_nothrow<IMFMediaEvent> Event(IMFMediaEventGenerator* source, MediaEventType expected)
{
    ...
    while (std::chrono::steady_clock::now() < end)
    {
        ...
        if (result == MF_E_NO_EVENTS_AVAILABLE) { std::this_thread::sleep_for(std::chrono::milliseconds(2)); continue; }
        Check(SUCCEEDED(result) && event, "camera source event is available within the fixture deadline");
        MediaEventType type{}; HRESULT status = E_FAIL;
        Check(SUCCEEDED(event->GetType(&type)) && SUCCEEDED(event->GetStatus(&status)) && SUCCEEDED(status), "camera event status succeeds");
...
wil::com_ptr_nothrow<IMFSample> Sample(IMFMediaStream* stream)
{
    ...
    Check(SUCCEEDED(event->GetValue(&value)) && value.vt == VT_UNKNOWN && value.punkVal, "media sample event carries a COM sample");
...
void CheckFrame(IMFSample* sample, BYTE luma, BYTE chroma)
{
    ...
    Check(SUCCEEDED(sample->GetBufferByIndex(0, buffer.put())) && SUCCEEDED(buffer.query_to(plane.put())), "camera frame uses standard MF 2D buffer");
    BYTE* bytes = nullptr; BYTE* start = nullptr; DWORD capacity = 0; LONG pitch = 0;
    Check(SUCCEEDED(plane->Lock2DSize(MF2DBuffer_LockFlags_Read, &bytes, &pitch, &start, &capacity)), "camera fixture locks output pixels");
    const auto unlock = wil::scope_exit([&] { (void)plane->Unlock2D(); });
    Check(pitch >= static_cast<LONG>(FrameWidth) && static_cast<uint64_t>(pitch) * FrameHeight * 3 / 2 <= capacity, "camera NV12 output is bounded and pitched");
    bool correct = true;
    for (UINT row = 0; row < FrameHeight * 3 / 2; ++row)
        for (LONG column = 0; column < pitch; ++column)
            correct = correct && bytes[static_cast<size_t>(row) * pitch + column] == (row < FrameHeight ? luma : chroma);
...
class SyntheticCapture final : public CaptureSession
{
  public:
    ...
    {
        ++_counts->opens;
        if (FAILED(_counts->failure.load())) return _counts->failure.load();
        try
        {
            auto frames = std::make_shared<Frames>(); frames->luma = id.View() == "camera-one" ? BYTE{235} : BYTE{60};
            ...
            _counts->active = SUCCEEDED(result); return result;
        }
        catch (const std::bad_alloc&) { return E_OUTOFMEMORY; }
    }
    void Close() noexcept override
    {
        if (_reader.IsOpen()) ++_counts->closes;
        _reader.Close(); _counts->active = false;
    }
...
uint32_t RunCameraControllerTests()
{
    ...
    DWORD needed = 0;
    Check(GetTokenInformation(token.get(), TokenUser, tokenData.data(), static_cast<DWORD>(tokenData.size()), &needed) != FALSE, "controller token SID");
    wil::unique_hlocal_string sid;
    Check(ConvertSidToStringSidW(reinterpret_cast<TOKEN_USER*>(tokenData.data())->User.Sid, sid.put()) != FALSE, "controller canonical SID");
    GUID guid{}; BridgeIdentity identity;
    Check(SUCCEEDED(CoCreateGuid(&guid)) && MakeBridgeIdentity(sid.get(), identity, &guid) == S_OK, "controller isolates its route namespace");
    ...
    DeviceId one, two; Check(one.Assign("camera-one") && two.Assign("camera-two"), "controller fixture source identities");
    ...
    Check(controller.Enable(true, state.revision - 1) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "controller rejects stale on intent");
    ...
    auto drive = [&](BYTE expected) {
        bool found = false; const auto deadline = GetTickCount64() + 2000;
        while (!found && GetTickCount64() < deadline)
        {
            const HRESULT result = consumer.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime());
            consumer.EndFrame();
    ...
    while (counts->active && GetTickCount64() < pauseDeadline) std::this_thread::sleep_for(std::chrono::milliseconds(5));
    ...
    state = controller.Snapshot();
    Check(controller.Select(one, state.revision) == E_ACCESSDENIED, "capture privacy failure is reported without a success acknowledgement");
    state = controller.Snapshot();
    Check(!state.enabled && state.availability == Availability::AccessDenied && !counts->active, "failed source acquisition closes capture and reports privacy state");
    drive(16);
    counts->failure = S_OK;
    ...
    Check(FAILED(consumer.Fill(output.data(), static_cast<DWORD>(output.size()), FrameWidth, MFGetSystemTime())),
        "retired off route requests full neutral fallback from the media source"); consumer.EndFrame(); consumer.Stop();
...
uint32_t RunCameraCaptureTests()
{
    ...
    const auto cleanup = wil::scope_exit([&] { reader.Close(); if (source) (void)source->Shutdown(); });
    Check(reader.OpenSource(source.get(), wake.get()) == S_OK && reader.IsOpen(), "asynchronous capture negotiates 720p NV12 through real Source Reader");
    Check(reader.OpenSource(nullptr, wake.get()) == E_INVALIDARG && reader.IsOpen(), "invalid capture input preserves the active reader");
    std::vector<BYTE> output(static_cast<size_t>(FrameWidth) * FrameHeight * 3 / 2);
    LONGLONG last = 0;
    for (uint32_t i = 0; i < 6; ++i)
    {
        HRESULT result = S_FALSE; LONGLONG timestamp = 0;
        ...
        Check(output.front() == 235 && output[FrameWidth * FrameHeight - 1] == 235 &&
            output[FrameWidth * FrameHeight] == 100 && output.back() == 100, "capture copies both NV12 planes without retained padding");
        Check(timestamp > last, "capture frame receipts have monotonic QPC times"); last = timestamp;
    }
    ...
    Check(reader.ReadFrame(std::span<BYTE>(output.data(), 10), timestamp) == E_INVALIDARG, "capture rejects an undersized destination");
    ...
    {
        source.reset();
        Check(CreateMediaSource(frames, source.put()) == S_OK && reader.OpenSource(source.get(), wake.get()) == S_OK, "capture restarts with a new callback session");
        reader.Close();
    }
...
uint32_t RunCameraMediaTests()
{
    ...
    const auto com = wil::scope_exit([] { CoUninitialize(); });
    Check(SUCCEEDED(MFStartup(MF_VERSION, MFSTARTUP_FULL)), "camera fixture starts Media Foundation without opening devices");
    const auto mf = wil::scope_exit([] { (void)MFShutdown(); });
    const uint32_t before = ActiveMediaObjects();
    auto provider = std::make_shared<Frames>();
    wil::com_ptr_nothrow<IMFMediaSource> source;
    Check(CreateMediaSource(provider, source.put()) == S_OK, "standalone camera media source constructs without registration or physical camera");
    const auto cleanup = wil::scope_exit([&] { if (source) (void)source->Shutdown(); });
    Check(provider->starts == 0 && provider->fills == 0, "metadata construction creates no capture work");
    DWORD flags = 0;
    Check(source->GetCharacteristics(&flags) == S_OK && flags == MFMEDIASOURCE_IS_LIVE, "camera source is live and unseekable");
    ...
    Check(SUCCEEDED(source.query_to(extended.put())) && SUCCEEDED(source.query_to(allocator.put())), "camera exposes Frame Server source and allocator mechanisms");
    DWORD input = 99; MFSampleAllocatorUsage usage{};
    Check(allocator->GetAllocatorUsage(0, &input, &usage) == S_OK && input == 0 && usage == MFSampleAllocatorUsage_UsesProvidedAllocator,
        "Frame Server supplies the bounded sample allocator");
    Check(allocator->GetAllocatorUsage(1, &input, &usage) == MF_E_INVALIDSTREAMNUMBER, "camera rejects unknown output stream");
    ...
    DWORD count = 0; BOOL selected = FALSE;
    ...
    Check(source->Start(presentation.get(), &GUID_NULL, &position) == S_OK, "camera source starts its consumer lifetime");
    auto newStream = Event(source.get(), MENewStream);
    wil::unique_prop_variant streamValue;
    Check(newStream->GetValue(&streamValue) == S_OK && streamValue.vt == VT_UNKNOWN, "new-stream event carries standard stream object");
    wil::com_ptr_nothrow<IMFMediaStream> stream;
    Check(SUCCEEDED(streamValue.punkVal->QueryInterface(IID_PPV_ARGS(stream.put()))), "stream exposes inherited IMFMediaStream");
    (void)Event(source.get(), MESourceStarted); (void)Event(stream.get(), MEStreamStarted);
    Check(provider->starts == 1 && provider->fills == 0, "running source waits for a consumer request instead of periodic capture");
    wil::com_ptr_nothrow<IUnknown> token; token.attach(new Token());
    ...
    Check(first->GetUnknown(MFSampleExtension_Token, IID_PPV_ARGS(sampleToken.put())) == S_OK && sampleToken.get() == token.get(),
        "camera preserves the consumer sample token");
    LONGLONG firstTime = 0, duration = 0;
    Check(first->GetSampleTime(&firstTime) == S_OK && firstTime > 0 && first->GetSampleDuration(&duration) == S_OK && duration == FrameDuration,
        "camera samples carry QPC-based time and duration");
    first.reset(); provider->failed = true;
    ...
    Check(off->GetSampleTime(&secondTime) == S_OK && secondTime >= firstTime + FrameDuration, "sample timestamps respect the 30 FPS ceiling");
    Check(FAILED(off->GetUnknown(MFSampleExtension_Token, IID_PPV_ARGS(sampleToken.put()))), "sample pool reuse clears an old request token");
    off.reset();
    ...
    (void)Event(stream.get(), MEStreamStopped); (void)Event(source.get(), MESourceStopped);
    Check(provider->stops == 1 && stream->RequestSample(nullptr) == MF_E_INVALIDREQUEST, "stopped camera releases transport and rejects sampling");
    Check(source->Start(presentation.get(), nullptr, &position) == S_OK, "camera can restart without re-registering the virtual device");
    (void)Event(source.get(), MEUpdatedStream); (void)Event(source.get(), MESourceStarted); (void)Event(stream.get(), MEStreamStarted);
    ...
    streamValue.reset(); newStream.reset(); stream.reset(); extended.reset(); allocator.reset(); source.reset();
    const auto end = std::chrono::steady_clock::now() + std::chrono::seconds(2);
    while (ActiveMediaObjects() != before && std::chrono::steady_clock::now() < end) std::this_thread::sleep_for(std::chrono::milliseconds(2));
...
uint32_t RunCameraChannelTests()
{
    checks = 0;
    wil::unique_handle token;
    Check(OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, token.put()) != FALSE, "camera IPC fixture reads its own identity");
    alignas(TOKEN_USER) std::array<BYTE, sizeof(TOKEN_USER) + SECURITY_MAX_SID_SIZE> tokenBytes{};
    DWORD returned = 0;
    Check(GetTokenInformation(token.get(), TokenUser, tokenBytes.data(), static_cast<DWORD>(tokenBytes.size()), &returned) != FALSE,
    ...
    Check(consumer.Create(ownerSid.get(), true) == S_OK, "camera test creates private local frame channel with explicit ACL");
    ...
    Check(rejected.Open(mismatched, true) == E_INVALIDARG, "camera frame mapping cannot be paired with a different lock");
    Check(producer.Open(consumer.Names(), true) == S_OK, "producer opens existing channel without creating global objects");
    ChannelState state;
    Check(producer.State(state) == S_OK && !state.enabled && state.revision == 1, "camera channel starts closed without an old image");
    ...
    const auto read = [&](LONGLONG now, bool live) {
        Check(consumer.Read(output.data(), static_cast<DWORD>(output.size()), pitch, now) == S_OK, "consumer takes frame-publication lease");
        const auto release = wil::scope_exit([&] { consumer.EndRead(); });
        bool correct = true;
        for (UINT row = 0; row < FrameHeight * 3 / 2; ++row)
            for (LONG column = 0; column < pitch; ++column)
            {
                const BYTE expected = live && column < static_cast<LONG>(FrameWidth) ? 201 : row < FrameHeight ? 16 : 128;
    ...
    Check(producer.Write(1, physical, 1'000'000) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "closed camera gate rejects physical frame publication");
    ...
    Check(consumer.Read(output.data(), static_cast<DWORD>(output.size()), pitch, 1'000'001) == S_OK, "consumer holds its lease through sample publication");
    HRESULT offResult = S_OK;
    std::thread off([&] { offResult = producer.SetGate({3, false}); });
    off.join();
    Check(offResult == HRESULT_FROM_WIN32(ERROR_TIMEOUT), "Off never acknowledges while an older sample is still publishing");
    consumer.EndRead();
    Check(producer.SetGate({3, false}) == S_OK, "Off acknowledges after the outstanding publication lease ends");
    Check(producer.Write(2, physical, 2'000'000) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "late old-source callback cannot cross a newer off gate");
    ...
    read(2'000'001, false);
    Check(producer.SetGate({4, true}) == HRESULT_FROM_WIN32(ERROR_REVISION_MISMATCH), "old source state cannot overwrite a newer binding");
    Check(producer.Write(5, std::span(physical).first(100), 3'000'000) == E_INVALIDARG, "frame transport rejects truncated input");
    Check(consumer.Read(output.data(), 10, pitch, 3'000'001) == E_INVALIDARG, "frame transport rejects undersized output before acquiring its gate");
    ...
    std::thread abandoned([&] {
        ...
        if (mutex) (void)WaitForSingleObject(mutex.get(), 1000);
        // Deliberately exit this fixture thread while owning only its test mutex.
    });
    abandoned.join();
    Check(producer.State(state) == HRESULT_FROM_WIN32(ERROR_ABANDONED_WAIT_0), "abandoned frame publication reports connection uncertainty");
