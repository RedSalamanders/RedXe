...
uint32_t RunInventorySelectionTests()
{
    ...
    for (uint32_t i = 80; i < 88; ++i) Check(preferences->outputs.Add(Id(i)), "late profile device admitted as a preference");
    auto inventory = std::make_unique<Inventory>();
    RankedInventory selection(inventory->outputs, inventory->outputCount);
    for (uint32_t i = 0; i < 100; ++i)
    {
        Endpoint row; row.id = Id(i);
        selection.Offer(row, InventoryRank(row.id, defaults, preferences->outputs));
    }
    Check(inventory->outputCount == MaximumOutputs && selection.Truncated(), "oversized enumeration stays bounded and reports overflow");
    for (const auto& id : defaults) Check(Contains(inventory->outputs, inventory->outputCount, id), "all three late audio defaults survive admission");
    for (uint32_t i = 80; i < 88; ++i) Check(Contains(inventory->outputs, inventory->outputCount, Id(i)), "late saved bindings displace ordinary endpoints");
    for (uint32_t i = 0; i < 21; ++i) Check(Contains(inventory->outputs, inventory->outputCount, Id(i)), "ordinary tie retains earliest enumeration order");
    Endpoint duplicate; duplicate.id = defaults[0]; selection.Offer(duplicate, 0);
    ...
    for (const auto& id : changedDefaults) Check(Contains(inventory->outputs, inventory->outputCount, id), "new default membership replaces still-present former defaults");
    for (const auto& id : defaults) Check(!Contains(inventory->outputs, inventory->outputCount, id), "evicted active endpoints can release watch slots before replacement");

    // More references than the final list can hold still preserve all defaults and the earliest references.
    for (uint32_t i = 0; i < 100; ++i) (void)preferences->inputs.Add(Id(i));
    ...
    for (const auto& id : defaults) Check(Contains(inventory->inputs, inventory->inputCount, id), "defaults outrank a full set of profile references");
    Check(Contains(inventory->inputs, inventory->inputCount, Id(28)) && !Contains(inventory->inputs, inventory->inputCount, Id(29)),
        "reference overflow has deterministic priority instead of identity substitution");

    for (uint32_t i = 20; i < 24; ++i) (void)preferences->cameras.Add(Id(i));
    RankedInventory cameras(inventory->cameras, inventory->cameraCount);
    const std::array selected{Id(29)};
    for (uint32_t i = 0; i < 30; ++i)
    {
        CameraDescriptor row; row.id = Id(i);
        cameras.Offer(row, InventoryRank(row.id, selected, preferences->cameras));
    }
    Check(inventory->cameraCount == MaximumCameras && cameras.Truncated(), "camera admission has its independent smaller bound");
    Check(Contains(inventory->cameras, inventory->cameraCount, selected[0]), "selected late camera cannot be evicted by another reference");
    for (uint32_t i = 20; i < 24; ++i) Check(Contains(inventory->cameras, inventory->cameraCount, Id(i)), "late camera profile references survive");
    ...
    *preferences = {};
    ...
    {
        Profile profile; profile.outputId = profile.microphoneId = profile.cameraId = Id(i);
